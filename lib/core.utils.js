...
export async function progressPush(sessionId, runId, step) {
  ...
  // Calculate elapsed time from start (like the old system)
  const now = Date.now();
  const startTime = state.meta.startedAt || now;
  const elapsedMs = now - startTime;
  
  const payload = { 
    ...step,
    timestamp: elapsedMs,  // Elapsed time in milliseconds (for frontend)
    at: now  // Absolute timestamp (for debugging)
  };
