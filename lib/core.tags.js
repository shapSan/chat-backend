...
export function normalizePartnership(src = {}) {
  ...
  return {
    // CRITICAL: Always include title/partnership_name for proper project identification
    title: src.title ?? src.partnership_name ?? src.name,
    partnership_name: src.partnership_name ?? src.title ?? src.name,
    ...
    main_cast: src.main_cast ?? src.stars ?? src.talent,
    synopsis: src.synopsis ?? src.logline ?? src.description,
    vibe: src.vibe ?? src.genre_production ?? src.genre ?? src.genres,
    partnership_setting: src.partnership_setting ?? src.setting ?? null
  };
}
