...
function isValidCast(value) {
  ...
  // If it's longer than 200 characters, it's probably not a cast list
  if (value.length > 200) {
...
export function normalizePartnership(src = {}) {
  ...
  // CRITICAL: It now intelligently skips placeholder values like "TBD"
  
  // Extract cast field - ONLY from main_cast or talent fields, NOT from generic 'cast' or 'stars'
  const rawCast = firstMeaningful(src.main_cast, src.talent);
  const validatedCast = isValidCast(rawCast) ? rawCast : null;
  
  if (rawCast && !validatedCast) {
    console.log('[normalizePartnership] Rejected invalid cast data:', rawCast.substring(0, 100));
  }
  
  return {
    // Project name - skip "TBD" and other placeholders
    ...
    // CRITICAL: Only use main_cast or talent fields - no 'cast' or 'stars' that might contain synopsis
    main_cast: validatedCast,
    cast: validatedCast,  // Also set 'cast' field for compatibility
