...
export async function searchWeb(query, options = {}) {
  ...
  try {
    // Use OpenAI to search and synthesize web information
    const searchPrompt = `You are a market research assistant with access to current information up to your knowledge cutoff.

Search Query: ${query}

Provide the 3 most relevant and recent pieces of information about this query. Focus on:
- Recent marketing campaigns or partnerships
- Brand activations or sponsorships
- Industry news or announcements
- Partnership deals or collaborations

For each result, provide:
1. A brief snippet (2-3 sentences) describing the information
2. The approximate date or timeframe
3. Key details that would be relevant for brand partnership decisions

Format as JSON array with this structure:
{
  "results": [
    {
      "snippet": "Brief description of the news/campaign",
      "date": "Approximate date or timeframe",
      "url": "example.com",
      "relevance": "Why this matters for brand partnerships"
    }
  ]
}

If you don't have specific information about recent campaigns, provide general industry insights about the brand's marketing approach and typical partnership strategies.`;
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { 
            role: 'system', 
            content: 'You are a market research assistant. Provide accurate, recent information about brands and their marketing activities. If you don\'t have recent specific information, provide general insights about the brand\'s typical marketing strategies.'
          },
    ...
    if (!response.ok) {
      console.error('[searchWeb] OpenAI API error:', response.status);
    ...
    try {
      ...
      if (parsed.results && Array.isArray(parsed.results)) {
        ...
        const limitedResults = parsed.results.slice(0, options.limit || 3);
        
        // Add fallback URLs if not provided
        const resultsWithUrls = limitedResults.map(r => ({
          snippet: r.snippet || 'No details available',
          url: r.url || `https://www.google.com/search?q=${encodeURIComponent(query)}`,
          date: r.date || 'Recent',
      ...
      console.error('[searchWeb] JSON parse error:', parseError);
    ...
    console.error('[searchWeb] Exception:', error.message);
