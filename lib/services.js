...
/* ---------------------------------------------------------
 * Nano Banana Image Generation (Google Gemini 2.5 Flash Image)
 * ------------------------------------------------------- */
export async function generateNanoBananaImage({
  prompt,
  sessionId,
  imageUrl, // Optional: for editing existing images
  referenceImage, // Optional: for using a reference image style/composition
  projectName,
  brandName,
  slideContent,
}) {
  ...
  try {
    // NEW: Handle reference image case (only when editing an existing image)
    if (referenceImage && imageUrl) {
      console.log('[Nano Banana] Editing image WITH reference:', { imageUrl, referenceImage });
      
      // Fetch both the existing image and reference image
      const existingImageResponse = await fetch(imageUrl);
      ...
      const existingImageArrayBuffer = await existingImageResponse.arrayBuffer();
      const existingImageBase64 = Buffer.from(existingImageArrayBuffer).toString('base64');
      const existingImageMimeType = existingImageResponse.headers.get('content-type') || 'image/png';
      
      const refArrayBuffer = await refResponse.arrayBuffer();
      const refBase64 = Buffer.from(refArrayBuffer).toString('base64');
      const refMimeType = refResponse.headers.get('content-type') || 'image/png';

      const apiUrl = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-image:generateContent?key=${googleGeminiApiKey}`;
        ...
        body: JSON.stringify({
          contents: [{
            parts: [
              ...
              {
                text: `Edit the first image using style/elements from the second reference image. ${prompt}`
              }
            ]
          }],
          generationConfig: {
      ...
      if (!response.ok) {
        const errorText = await response.text();
        console.error('[Nano Banana Reference Edit] API error:', response.status, errorText);
        throw new Error(`Image editing with reference failed: ${response.status} - ${errorText}`);
      }
      ...
      if (!imageData?.inlineData?.data) {
        console.error('[Nano Banana Reference Edit] No image in response:', JSON.stringify(data, null, 2));
      ...
      const filename = `${sessionId || 'unknown-session'}/nano-banana-ref-edit-${timestamp}.png`;
      ...
      console.log('[Nano Banana Reference Edit] Image edited successfully:', url);

      return {
        success: true,
        imageUrl: url,
        model: 'nano-banana-reference-edit',
        storage: 'blob',
        prompt,
        usedReference: true,
      };
    ...
    // For text-to-image generation using Gemini (no reference)
    if (!imageUrl && !referenceImage) {
      ...
      console.log('[Nano Banana] Image generated successfully:', url);
    ...
    // For editing existing images (image-to-image)
      ...
      // Fetch the existing image to send to Gemini
      ...
      // Use Gemini for image editing with both the image and edit prompt
      const apiUrl = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-image:generateContent?key=${googleGeminiApiKey}`;
      ...
      if (!response.ok) {
        const errorText = await response.text();
        console.error('[Nano Banana Edit] API error:', response.status, errorText);
      ...
      const editedImageData = data.candidates?.[0]?.content?.parts?.find((part) => part.inlineData);
      
      if (!editedImageData?.inlineData?.data) {
        console.error('[Nano Banana Edit] No image in response:', JSON.stringify(data, null, 2));
        throw new Error('No edited image generated in response');
      }

      // Convert base64 to buffer
      const mimeType = editedImageData.inlineData.mimeType || 'image/png';
      const base64Data = editedImageData.inlineData.data;
      const imageBuffer = Buffer.from(base64Data, 'base64');
      ...
      console.log('[Nano Banana Edit] Image edited successfully:', url);
