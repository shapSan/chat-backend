...
export async function generateWildcardBrands(synopsis) {
  if (!openAIApiKey) return [];
  try {
    const systemPromptContent = `You are an expert brand integration strategist. Your task is to suggest exactly 5 UNEXPECTED but brilliant "wild card" consumer brand partnerships for the given production synopsis.

CRITICAL RULES:
1.  Think outside the box. Avoid obvious choices. Find surprising but perfect fits.
2.  For EACH brand, you MUST provide a concise, specific reason (max 15 words) explaining the creative synergy.
3.  Your ONLY output must be a single, valid JSON object in this exact format: {"brands": ["Brand Name - Specific reason for the fit.", "Another Brand Name - Another specific reason."]}

Example Synopsis: A romantic comedy about bakers in Paris.
Example JSON Output: {"brands": ["Le Creuset - High-end cookware for authentic baking scenes.", "Vespa - Iconic Parisian transport for romantic montages."]}`;
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: 'user', content: `Production Synopsis: ${synopsis.slice(0, 500)}` },
        ],
        response_format: { type: 'json_object' },
        temperature: 0.9,
        max_tokens: 150,
