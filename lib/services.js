...
export async function generateNanoBananaImage({
  ...
  try {
    // NEW: Handle reference image case (generate new image using reference style)
    if (referenceImage && !imageUrl) {
      console.log('[Nano Banana] Generating image WITH reference:', referenceImage);
        ...
        body: JSON.stringify({
          contents: [{
            parts: [
              ...
              {
                text: `Using this reference image as style/composition inspiration, create a new image: ${prompt}`
              }
            ]
          }],
          generationConfig: {
      ...
      if (!response.ok) {
        const errorText = await response.text();
        console.error('[Nano Banana Reference] API error:', response.status, errorText);
        throw new Error(`Image generation with reference failed: ${response.status} - ${errorText}`);
      }
      ...
      if (!imageData?.inlineData?.data) {
        console.error('[Nano Banana Reference] No image in response:', JSON.stringify(data, null, 2));
      ...
      const filename = `${sessionId || 'unknown-session'}/nano-banana-ref-${timestamp}.png`;
      ...
      console.log('[Nano Banana Reference] Image generated successfully:', url);

      return {
        success: true,
        imageUrl: url,
        model: 'nano-banana-reference',
        storage: 'blob',
        prompt,
        usedReference: true,
      };
