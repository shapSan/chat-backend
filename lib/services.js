...
export async function searchWeb(query, options = {}) {
  ...
  if (anthropicApiKey) {
    try {
      ...
      if (liveResults.results?.length > 0) {
        console.log('[searchWeb] Claude returned', liveResults.results.length, 'results');
        results.push(...liveResults.results);
      }
    } catch (error) {
      console.error('[searchWeb] Claude search failed:', error.message);
    }
  }
  ...
  if (openAIApiKey) {
    try {
      ...
      if (strategicResults.results?.length > 0) {
        console.log('[searchWeb] OpenAI returned', strategicResults.results.length, 'results');
        results.push(...strategicResults.results);
      }
    } catch (error) {
      console.error('[searchWeb] OpenAI search failed:', error.message);
    }
  }
  ...
  if (results.length === 0) {
    console.log('[searchWeb] No results found from any source');
    return { results: [], error: 'No API keys configured or search failed' };
  }
  ...
  console.log('[searchWeb] Returning', uniqueResults.length, 'unique results');
