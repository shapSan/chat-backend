 ...
 */
export function logStage(stage, obj, keys, extra = {}) {
  ...
  // If keys not provided, use partnership fields as default
  const keysToUse = keys || HB_KEYS.PARTNERSHIP_FIELDS;
  
  const flat = pick(obj, keysToUse);
  const props = pick(obj.properties ?? {}, keysToUse);

  const parts = keysToUse.map((k) => {
    const p = props[k];
    const t = flat[k];
