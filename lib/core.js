...
async function fetchAllBrandData(intent, { add, knownProjectName, userMessage, partnershipData }) {
  ...
  if (openAIApiKey && (partnershipData?.vibe || partnershipData?.synopsis || search_term)) {
    try {
      ...
      const categoryPrompt = knownProjectName 
        ? `Based on the project '${knownProjectName}', which is a ${contextForCategories}, provide the top 2 most likely search terms that would appear in the title or summary of a relevant business meeting. Prioritize specific sub-genres or related industries. For example, for an 'Action Thriller', good terms might be 'Stunt Coordination' or 'Automotive Placement'. Respond with only a comma-separated list.`
        : `Based on this production context, provide the top 2 most likely search terms for finding relevant business meetings. Focus on specific industry terms. Context: ${contextForCategories.slice(0, 300)}`;
      ...
      if (categoryResponse.ok) {
        ...
        if (categoriesText) {
          businessCategories = categoriesText.split(',').map(cat => cat.trim()).slice(0, 2);
  ...
  const categoryResults = uniqueCategoryTerms.length > 0 && hubspotResults[0]?.status === 'fulfilled' 
  ...
  const startIdx = uniqueCategoryTerms.length > 0 ? 1 : 0;
  const endIdx = startIdx + Math.min(nameTerms.length, 4);
  ...
  // Process active clients results
  ...
  add({ type: 'result', text: `📊 HubSpot Found: ${categoryResults.results?.length || 0} category matches, ${nameSearchResults.length} name matches, ${activeResults.results?.length || 0} active clients.` });
