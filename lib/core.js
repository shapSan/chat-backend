...
export async function handleClaudeSearch(
  ...
  switch (intent.tool) {
    ...
    case 'create_pitches_for_brands': {
      ...
      // Also check if the brands are coming from the message itself
      ...
      for (const brandInput of brands) {
        ...
        if (/^\d{10,}$/.test(trimmedInput)) {
          ...
          try {
            // First try to get the company directly by ID
            const url = `https://api.hubapi.com/crm/v3/objects/companies/${trimmedInput}`;
            ...
            if (response.ok) {
              ...
              processedBrands.push({
                id: company.id,
                name: company.properties.brand_name || company.properties.name || `Brand ${trimmedInput}`,
                category: company.properties.main_category || 'Unknown',
                subcategories: company.properties.product_sub_category__multi_ || '',
                clientStatus: company.properties.client_status || '',
                clientType: company.properties.client_type || '',
                partnershipCount: company.properties.partnership_count || '0',
                dealsCount: company.properties.deals_count || '0'
              });
              add({ type: 'result', text: `✅ Found: ${company.properties.brand_name || company.properties.name}` });
            } else {
              // If direct ID lookup fails, try search
              ...
              if (searchResult?.results?.length > 0) {
                ...
                processedBrands.push({
                  id: brand.id,
                  name: brand.properties.brand_name || brand.properties.name || `Brand ${trimmedInput}`,
                ...
                add({ type: 'result', text: `✅ Found: ${brand.properties.brand_name || brand.properties.name}` });
              } else {
                // Can't find by ID - use as placeholder
                processedBrands.push({
                  id: trimmedInput,
                  name: `Unknown Brand (ID: ${trimmedInput})`,
                  category: 'Unknown',
                  isExternal: true
                });
          ...
          try {
            ...
            if (searchResult?.results?.length > 0) {
              ...
              processedBrands.push({
                id: brand.id,
                name: brand.properties.brand_name || brand.properties.name || trimmedInput,
                category: brand.properties.main_category || 'Unknown',
                subcategories: brand.properties.product_sub_category__multi_ || '',
                clientStatus: brand.properties.client_status || '',
                clientType: brand.properties.client_type || '',
              ...
              add({ type: 'result', text: `✅ Found: ${brand.properties.brand_name || brand.properties.name} in HubSpot` });
            } else {
      ...
      add({ type: 'start', text: `🧠 Generating integration ideas for: ${processedBrands.map(b => b.name).join(', ')}` });
      ...
      const creativeResult = await generateCreativeContent(processedBrands.slice(0, 5), {
        ...
        search_term: `Generate brand integration ideas for: ${processedBrands.map(b => b.name).join(', ')}`,
        partnershipData: {
      ...
      return {
        mcpThinking,
        finalReplyText: creativeResult.finalReplyText || `Generated integration ideas for ${processedBrands.map(b => b.name).join(', ')}`,
        organizedData: {
          dataType: 'BRAND_RECOMMENDATIONS',
