...
\`\`\``;
  ...
  const buckets = [
    ...
    { name: 'Active Client', filter: b => b.tags?.includes('🔥'), quota: 4 },
    { name: 'Wild Card', filter: b => b.tags?.includes('🚀') || b.isWildcard, quota: 5 },
    { name: 'Discovery', filter: b => b.tags?.includes('🆕'), quota: 4 }
  ];
  ...
  for (const bucket of buckets) {
    ...
    const topFromBucket = bucketBrands.slice(0, bucket.quota);
  ...
  // The final list for the picker is now the diversified list
  ...
  // Step 4: Find the best "creative bet". This is the highest-scoring "Wild Card" that we haven't already picked.
  const creativeBet = allRankedBrands.find(b => 
    b.tags?.includes('🚀 Wild Card') && !usedIds.has(b.id)
  );
  if (creativeBet) {
...
/* =========================
 * MAIN HANDLER FOR CLAUDE SEARCH
 * ======================= */
export async function handleClaudeSearch(
  ...
  console.log('[handleClaudeSearch] Routed intent:', intent);
  ...
  switch (intent.tool) {
    case 'create_pitches_for_brands': {
      ...
      // --- NEW AND CRITICAL SAFETY CHECK ---
      // This block fixes the bug where the AI router only returns the first brand from a list.
      if (brands.length === 1 && userMessage.includes(',')) {
        const match = userMessage.match(/for:\s*(.+?)(?:\s+for|$)/i);
        if (match && match[1] && match[1].includes(',')) {
          console.log('[handleClaudeSearch] Router returned single brand from a list. Overriding with manual parse.');
          // Manually parse the comma-separated list from the original user message.
          brands = match[1].split(',').map(b => b.trim()).filter(Boolean);
          add({ type: 'info', text: `🧠 Corrected brand list: ${brands.join(', ')}` });
        }
      }
      // --- END OF FIX ---
      ...
      if (!projectContext && lastProductionContext) {
        ...
        if (projectMatch) projectContext = projectMatch[1].trim();
      }
      ...
      for (const brandInput of brands) {
        ...
        try {
            const searchResult = await hubspotAPI.searchBrands({
              query: trimmedInput,
              limit: 1,
              properties: ['brand_name', 'name', 'main_category', 'product_sub_category__multi_', 'client_status', 'client_type', 'partnership_count', 'deals_count']
            });
            if (searchResult?.results?.length > 0) {
              ...
              processedBrands.push({ id: `external-${trimmedInput.replace(/\s+/g, '-').toLowerCase()}`, name: trimmedInput, category: 'Unknown', isExternal: true });
              brandNamesForMessage.push(trimmedInput);
              add({ type: 'info', text: `ℹ️ "${trimmedInput}" not found, treating as external` });
            }
        } catch (error) {
            console.error(`[handleClaudeSearch] Error searching for brand "${trimmedInput}":`, error);
            processedBrands.push({ id: `error-${trimmedInput.replace(/\s+/g, '-').toLowerCase()}`, name: trimmedInput, category: 'Unknown', error: true });
            brandNamesForMessage.push(trimmedInput);
        }
      }

      if (processedBrands.length === 0) {
        return { mcpThinking, organizedData: { dataType: 'ERROR', message: 'No brands could be processed.' } };
      }
      ...
      if (projectContext && hubspotAPI?.getPartnershipForProject) {
        partnershipData = await hubspotAPI.getPartnershipForProject(projectContext).catch(() => null);
      }
      ...
      const creativeResult = await generateCreativeContent(processedBrands.slice(0, 5), {
        ...
        partnershipData: partnershipData || { title: projectContext, synopsis: lastProductionContext }
      });

      return {
        mcpThinking,
        finalReplyText: creativeResult.finalReplyText || `Generated integration ideas for ${brandNamesForMessage.join(', ')}.`,
        organizedData: {
    ...
    case 'find_brands': {
      // Run the brand search orchestrator
    ...
    case 'get_brand_activity': {
      // Handle brand activity search
      const { brand_name } = intent.args;
      add({ type: 'info', text: `🔍 Searching for activity about: ${brand_name}` });
      
      // Search for communications
      const searchTerms = [brand_name];
      const communications = await searchCommunicationsForClues(
        { title: brand_name }, 
        { add }
      );
    ...
    default:
      // General question - no MCP tools needed
      return null;
  }
}
