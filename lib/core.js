...
export async function runBrandSearchOrchestrator(
  ...
  // (autofix) Restored function wrapper around previously stray block.
  // Step 1 (reconstructed): seed minimal variables used later.
  let projectName = knownProjectName || (intent && intent.args && (intent.args.project_name || intent.args.production)) || '';
  try {
    if (!projectName) {
      const m = /(?:for|on|about)\s+["']?([^"']{3,60})["']?/i.exec(userMessage || '');
