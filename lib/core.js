...
export function tagAndCombineBrands({
  ...
  activityBrands?.results?.forEach((b) => {
    const contextData = findBrandContext(b.properties.brand_name || '');
    if (!brandMap.has(b.id)) {
      addBrandBase(b.id, b, '📧 Recent Activity', `Recent engagement - last activity ${new Date(b.properties.hs_lastmodifieddate).toLocaleDateString()}`, contextData);
    }
  });
  ...
  synopsisBrands?.results?.forEach((b) => {
    const contextData = findBrandContext(b.properties.brand_name || '');
    if (!brandMap.has(b.id)) {
      addBrandBase(b.id, b, '🎯 Genre Match', 'Matches production genre and themes', contextData);
    } else {
      const br = brandMap.get(b.id);
  ...
  genreBrands?.results?.forEach((b) => {
    const contextData = findBrandContext(b.properties.brand_name || '');
    if (!brandMap.has(b.id)) {
      addBrandBase(b.id, b, '🎭 Vibe Match', generatePitch(b.properties, synopsis), contextData);
    } else {
      const br = brandMap.get(b.id);
  ...
  activeBrands?.results?.forEach((b) => {
    ...
    if (!brandMap.has(b.id)) {
      addBrandBase(
        ...
        `Budget proven: ${b.properties.deals_count} deals, ${b.properties.partnership_count} partnerships`,
        contextData
      );
      const br = brandMap.get(b.id);
...
export async function handleClaudeSearch(
  ...
  try {
    switch (intent.tool) {
      case 'find_brands': {
        // STEP 1: EXTRACT THE CORRECT PROJECT NAME
        let actualProjectName = knownProjectName;
        
        if (!actualProjectName) {
          // Look for explicit project mentions with full name extraction
          const projectPatterns = [
            // Pattern for "for/about/called 'Project Name'"
            /(?:for|about|called|titled|project:?)\s*["']([^"']+)["']/i,
            // Pattern for "'Project Name' reboot/remake/etc"
            /["']([^"']+)["']\s*(?:reboot|remake|sequel|series|movie|film|show|production)/i,
            // Pattern for "movie/film called Project Name"
            /(?:movie|film|show|series|production)\s*(?:called|titled|named)?\s*["']([^"']+)["']/i,
            // Pattern for structured format "Title: Project Name"
            /(?:Title|Project|Film|Movie|Show):\s*([^\n]+)/i,
            // Pattern for "Project Name movie/film" without quotes
            /^([A-Z][^,\n]+?)\s+(?:movie|film|show|series|production|reboot|remake)/i
          ];
          ...
          // If still no name and there's a synopsis, try to extract from context
          if (!actualProjectName && genre) {
            // Check for a capitalized phrase that might be the title
            const titleMatch = userMessage.match(/^([A-Z][A-Za-z\s]+?)(?:\.|,|\n|$)/);
            if (titleMatch && titleMatch[1].length < 50) {
              actualProjectName = titleMatch[1].trim();
            } else {
              actualProjectName = `${genre.charAt(0).toUpperCase() + genre.slice(1)} Production`;
            }
          }
        ...
        // Full list for picker (minimal details)
        const brandSuggestionsForPicker = finalBrands.map(brand => ({
          id: brand.id,
          name: brand.name,
        ...
        // Top 2 brands for detailed cards
        const top2Brands = finalBrands.slice(0, 2);
        
        // STEP 4: EXECUTE THE CREATIVE AI CALL - DO IT NOW, NOT LATER
        add({ type: 'process', text: '✍️ Generating creative content for top 2 brands...' });
        
        // Build the prompt for the AI using Airtable format
        const airtablePrompt = `You are a brand partnership specialist. Generate creative integration ideas for these brands with the project "${actualProjectName}".

Project Synopsis: ${search_term}
Genre: ${genre || 'general'}
Distributor: ${distributorForTerms || 'Independent'}

${top2Brands.map((brand, idx) => `
Brand ${idx + 1}: ${brand.name}
Category: ${brand.category}
Status: ${brand.clientStatus}
Track Record: ${brand.partnershipCount} partnerships, ${brand.dealsCount} deals
${brand.insight?.rawText ? `Recent Activity: ${brand.insight.rawText}` : ''}
`).join('\n')}

For each brand, write:
- Integration: [Specific creative integration ideas for this production]
- Why it works: [Why this brand fits the production's themes and audience]
- HB Insights: [Data-driven insights from their partnership history]

Format each brand section starting with "Brand: [Name]" on its own line.`;

        // Call the AI to generate creative content
        ...
        if (openAIApiKey) {
          try {
            const aiResponse = await fetch('https://api.openai.com/v1/chat/completions', {
              ...
              body: JSON.stringify({
                ...
                messages: [
                  { role: 'system', content: 'You are a creative brand partnership specialist. Write compelling integration ideas.' },
                  { role: 'user', content: airtablePrompt }
                ],
                temperature: 0.7,
                max_tokens: 1000
              })
            ...
            console.error('[AI Creative Generation] error:', error?.message);
          }
          ...
          try {
            const aiResponse = await fetch('https://api.anthropic.com/v1/messages', {
              ...
              body: JSON.stringify({
                model: MODELS.anthropic.claude,
                messages: [{ role: 'user', content: airtablePrompt }],
                max_tokens: 1000,
                temperature: 0.7
              })
            });
        ...
        // STEP 5: PARSE THE CREATIVE TEXT AND POPULATE FINAL DATA
        let parsedBrands = [];
        
        if (creativeTextResponse) {
          // Parse the AI response by brand sections
          const brandSections = creativeTextResponse.split(/Brand:/i).filter(s => s.trim());
          
          parsedBrands = brandSections.slice(0, 2).map((section, idx) => {
            const lines = section.split('\n').filter(l => l.trim());
            const brandName = lines[0]?.trim() || top2Brands[idx]?.name || `Brand ${idx + 1}`;
            
            // Extract the specific content lines
            const content = [];
            for (const line of lines.slice(1)) {
              if (line.includes('Integration:') || line.includes('Why it works:') || line.includes('HB Insights:')) {
                content.push(line.trim());
              }
            }
            
            // If parsing failed, use enhanced defaults
            if (content.length < 3) {
              const brand = top2Brands[idx];
              content.push(
                `Integration: ${brand.category} partnership perfectly aligned with ${actualProjectName}'s narrative`,
                `Why it works: ${brand.reason || `Strong synergy with ${genre || 'production'} themes`}`,
                `HB Insights: ${brand.insight?.rawText || `${brand.partnershipCount} successful partnerships, ${brand.dealsCount} active deals`}`
              );
            }
          ...
          // Fallback if AI call failed - use enhanced defaults
          parsedBrands = top2Brands.map(brand => ({
            name: brand.name,
            content: [
              `Integration: ${brand.category} integration seamlessly woven into ${actualProjectName}'s storyline`,
              `Why it works: ${brand.reason || `Natural fit for ${genre || 'this'} production's target audience`}`,
              `HB Insights: ${brand.insight?.rawText || `Track record: ${brand.partnershipCount} partnerships, ${brand.dealsCount} deals, ${brand.clientStatus} status`}`
            ]
          }));
        }
        
        add({ type: 'complete', text: `✅ Creative content generated and parsed for ${parsedBrands.length} brands` });
