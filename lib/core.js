...
async function searchCommunicationsForClues(partnershipData, { add }) {
  ...
  // SIMPLIFIED: Only search for primary entities + get recent transcripts as fallback
  const primarySearchTerms = searchQueries.primary_entities.filter(Boolean);
  
  if (primarySearchTerms.length === 0 && partnershipData.title) {
    primarySearchTerms.push(partnershipData.title);
  }
  
  const allMeetings = new Map();
  const allEmails = new Map();
  
  // Search for primary entities only (project title, main talent)
  if (primarySearchTerms.length > 0) {
    add({ type: 'info', text: `🔍 Searching communications for: ${primarySearchTerms.slice(0, 3).join(', ')}` });
    const [ffResult, o365Result] = await Promise.allSettled([
      ...
      o365API.searchEmails ? o365API.searchEmails(primarySearchTerms, { limit: 10 }) : Promise.resolve({ emails: [] })
    ]);
