...
Return a single JSON object with the key "pitches", containing an array of objects. Each object should have keys "brandName", "integration", "whyItWorks", and "hbInsights".`;
  } else {
...
CRITICAL: Return ONLY a valid JSON object with a single key "pitches". The value should be an array of objects, where each object represents a brand and has the keys "brandName", "integration", "whyItWorks", and "hbInsights" based on the rules above.
`;
  ...
  try {
    const aiResponse = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'gpt-4o-mini',
        messages: [
          { role: 'system', content: 'You are a creative brand partnership specialist. Return only valid JSON as specified.' },
  ...
  let parsedPitches = [];
  try {
    const parsedJson = JSON.parse(finalFormattedText);
    if (parsedJson.pitches && Array.isArray(parsedJson.pitches)) {
      parsedPitches = parsedJson.pitches;
      console.log(`[generateCreativeContent] Successfully parsed ${parsedPitches.length} brand pitches from JSON.`);
    }
  } catch (error) {
    console.error('[generateCreativeContent] Failed to parse JSON from AI response:', error);
    // Will fall back to empty array and use fallback logic below
  }
  ...
  const enrichedBrandsWithContent = topBrands.map((originalBrand, index) => {
    ...
    // Try to find matching pitch from parsed JSON
    let matchedPitch = null;
    if (parsedPitches.length > 0) {
      // Try to match by name (case-insensitive)
      matchedPitch = parsedPitches.find(pitch => 
        pitch.brandName && pitch.brandName.toLowerCase().trim() === actualBrandName.toLowerCase().trim()
      );
      
      // If not found by name and we have the same number of pitches as brands, use index
      if (!matchedPitch && parsedPitches[index] && parsedPitches.length === topBrands.length) {
        console.log(`[generateCreativeContent] Using index-based match for brand ${actualBrandName} at index ${index}`);
        matchedPitch = parsedPitches[index];
      }
    }
    
    // Build content array from matched pitch or use fallback
    let parsedContent = [];
    if (matchedPitch) {
      parsedContent = [
        `Brand: ${matchedPitch.brandName || actualBrandName}`,
        `Integration: ${matchedPitch.integration || 'Strategic brand placement'}`,
        `Why it works: ${matchedPitch.whyItWorks || 'Natural fit with production'}`,
        `HB Insights: ${matchedPitch.hbInsights || 'Strong partnership potential'}`
      ];
    } else {
      // No matching pitch found - generate fallback
      console.warn(`[generateCreativeContent] No JSON content found for ${actualBrandName}, using fallback`);
...
export async function handleClaudeSearch(
  ...
  switch (intent.tool) {
    ...
    case 'create_pitches_for_brands': {
      ...
      // CRITICAL FIX: Use knownProjectName as the definitive project context
      let finalProjectName = knownProjectName; // Prioritize the name from the client
      let partnershipData = null;
      ...
      if (brands.length === 0) {
        ...
        if (forPattern) {
          ...
          // Don't override finalProjectName unless we don't have one
          if (!finalProjectName) {
            finalProjectName = forPattern[2].trim();
          }
        }
      ...
      console.log('[handleClaudeSearch] Using project:', finalProjectName);
      
      // First, check if the client sent us the complete data object
      if (knownPartnershipData && (knownPartnershipData.synopsis || knownPartnershipData.title)) {
        partnershipData = normalizePartnership(knownPartnershipData);
        finalProjectName = partnershipData.title || finalProjectName;
        add({ type: 'info', text: `✅ Using project context for "${finalProjectName}" from client.` });
        console.log('[handleClaudeSearch] Using partnership data provided by client.');
      } 
      // If not, use the knownProjectName to fetch the data
      else if (finalProjectName && hubspotAPI?.getPartnershipForProject) {
        try {
          add({ type: 'process', text: `🔍 Fetching project data for "${finalProjectName}" from HubSpot...` });
          const freshData = await hubspotAPI.getPartnershipForProject(finalProjectName);
          if (freshData) {
            partnershipData = normalizePartnership(freshData);
            add({ type: 'result', text: `✅ Found project data for "${finalProjectName}".` });
          } else {
            add({ type: 'warning', text: `⚠️ Could not find project data for "${finalProjectName}" in HubSpot.` });
          }
        } catch (error) {
          console.error(`[handleClaudeSearch] Error fetching partnership data for ${finalProjectName}:`, error);
        }
      }
      
      // CRITICAL: If after all checks, partnershipData is still null, create minimal fallback
      if (!partnershipData) {
        // Only exit with error if we have no project name at all
        if (!finalProjectName) {
          return {
            mcpThinking,
            organizedData: {
              dataType: 'ERROR',
              message: `Could not identify a valid project context. Please specify the project.`,
            },
          };
        }
        // Otherwise create minimal partnership data from what we have
        partnershipData = {
          title: finalProjectName,
          partnership_name: finalProjectName,
          synopsis: lastProductionContext || 'No synopsis available',
          genre_production: 'Not specified'
        };
        add({ type: 'warning', text: `⚠️ Using minimal project context for "${finalProjectName}".` });
      }
      ...
      const updatedUserMessage = `Generate brand integration ideas for: ${brandNamesForMessage.join(', ')}${finalProjectName ? ` for ${finalProjectName}` : ''}`;
      ...
      const creativeResult = await generateCreativeContent(enhancedBrands, {
        add,
        actualProjectName: finalProjectName || 'your project',
      ...
      return {
        mcpThinking,
        finalReplyText: creativeResult.finalReplyText || `Generated integration ideas for ${brandNamesForMessage.join(', ')}${finalProjectName ? ` for ${finalProjectName}` : ''}`,
        organizedData: {
          dataType: 'BRAND_RECOMMENDATIONS',
          detailedBrands: creativeResult.enrichedBrands || processedBrands,
          brandSuggestions: [],
          projectName: finalProjectName || knownProjectName,
          partnershipData: partnershipData,  // We now guarantee this exists
