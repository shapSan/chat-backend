...
async function getSemanticallyRankedBrands(partnershipData, { add }) {
  ...
  const rankingPrompt = `You are a creative director analyzing brands for perfect thematic alignment with a production.

Your ONLY job: Find brands that naturally fit the story, genre, and audience. IGNORE partnership counts and client status completely.
...
- Target Rating: ${partnershipData.rating || 'Not specified'}
</PRODUCTION_CONTEXT>
...
1. **Natural Thematic Fit (90% weight):** Does this brand's category/subcategories naturally belong in this story? Would audiences think "of course that brand is there"? Examples:
   - Action film → automotive, sports equipment, energy drinks
   - Romance → jewelry, fashion, cosmetics, restaurants
   - Family → toys, snacks, theme parks, family restaurants
   - Horror → security systems, flashlights, insurance (ironic placement)
   - Comedy → everyday products used in funny ways

2. **Demographic Match (10% weight):** Does the brand's typical customer match who will watch this?
   - Teen comedy → brands teens actually use
   - Mature drama → premium/luxury brands
   - Family film → wholesome, multi-generational brands

3. **COMPLETELY IGNORE:**
   - Partnership counts (irrelevant!)
   - Client status (don't care if active/inactive!)
   - Number of deals (meaningless!)
   - Only consider: Does this brand NATURALLY FIT the creative?

<OUTPUT_INSTRUCTIONS>
- Return a JSON array of the TOP 50 brands ranked PURELY by creative fit
- Each object: {"id": "<brand_id>", "score": <0-100>, "reason": "<Why this CREATIVELY fits the story>"}
- High scores = perfect natural fit. Low scores = forced/unnatural fit.
- Your reasons must explain the CREATIVE connection, never mention partnership history!
`;
...
\`\`\``;
  ...
  // --- CREATIVE-FIRST SELECTION LOGIC ---
  // Select the top 2 brands based PURELY on creative fit scores
  // Ignore client status - just pick the best creative matches!
  ...
  // The AI has already ranked them by creative fit, so trust that!
  ...
  // --- END OF CREATIVE-FIRST SELECTION LOGIC ---
