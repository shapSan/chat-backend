...
async function generateCreativeContent(topBrands, { add, actualProjectName, search_term, partnershipData, communications }) {
  ...
  const enrichedBrandsWithContent = topBrands.map((originalBrand) => {
    ...
    // Use the data directly from the LITE deep dive.
    const integrationIdeas = insights.integrationIdeas || 'No specific ideas generated.';
    const whyItWorks = insights.strategicFit || insights.whyItWorks || 'Aligns with production themes.'; // In LITE mode, strategicFit holds this value.
