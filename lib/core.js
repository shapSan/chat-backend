...
export function tagAndCombineBrands({
  ...
  if (wildcardCategories?.length) {
    wildcardCategories.slice(0, 5).forEach((brandString, idx) => {
      ...
      brandMap.set(`wildcard_${idx}`, {
        ...
        tags: ['🚀 Net-New', '💡 Creative Suggestion', '🎭 Trending'],
  ...
  const buckets = [
    ...
    { tag: '🚀 Net-New', count: 5 },                // Renamed from Cold Outreach
    { tag: '🚀 Net-New Opportunity', count: 5 }     // Alternative tag name
  ];
...
async function searchCommunicationsForClues(partnershipData, { add }) {
  ...
  if (primarySearchTerms.length > 0) {
    ...
    for (const term of primarySearchTerms) {
      ...
      const escapedTerm = term.replace(/[.*+?^${}()|[\]\\]/g, '\\  if (primarySearchTerms.length > 0) {
    add({ type: 'info', text: `🔍 Searching comms for: ${primarySearchTerms.join(', ')}` });
    const DAYS = 90;
    const fromDate = new Date(Date.now() - DAYS * 24 * 60 * 60 * 1000).toISOString();
    ...
    for (const term of primarySearchTerms) {
      ...
      if (ffResult.status === 'fulfilled' && ffResult.value.transcripts) {
        ffResult.value.transcripts.forEach(t => {
          const textToSearch = `${t.title || ''} ${t.summary?.overview || ''} ${t.summary?.keywords?.join(' ') || ''}`;
          // Only add if whole word match is found
          if (wholeWordRegex.test(textToSearch)) {
            if (!allMeetings.has(t.id)) {
              allMeetings.set(t.id, { ...t, matchedKeyword: term });
            }
          }
      ...
      if (o365Result.status === 'fulfilled' && o365Result.value.emails) {
        o365Result.value.emails.forEach(e => {
          const textToSearch = `${e.subject || ''} ${e.preview || ''}`;
          // Only add if whole word match is found
          if (wholeWordRegex.test(textToSearch)) {
            if (!allEmails.has(e.id)) {
              allEmails.set(e.id, { ...e, matchedKeyword: term });
  ...
  // If no results, DON'T show fallback meetings - be honest
  if (allMeetings.size === 0 && firefliesApiKey) {
    add({ type: 'info', text: '🔍 No meetings found mentioning this project' });
    // Don't add any fallback meetings - just return empty results
  }
...
async function generateCreativeContent(topBrands, { add, actualProjectName, search_term, partnershipData, communications }) {
  ...
  // --- NEW: Find specific evidence for each brand with extracted snippets ---
  const brandsWithEvidence = topBrands.map(brand => {
    let evidence = { source: null, snippet: null }; // Change evidence to an object
    const brandNameLower = (brand.name || '').toLowerCase();
    const wholeWordRegex = new RegExp(`\\b${brandNameLower.replace(/[.*+?^${}()|[\]\\]/g, '\\  // --- NEW: Find specific evidence for each brand ---
  const brandsWithEvidence = topBrands.map(brand => {
    let evidence = null;
    const brandNameLower = (brand.name || '').toLowerCase();

    // Check for actual mentions in meetings
    ...
    // Check for actual mentions in emails
    ...
    // No evidence found - let the AI explain the natural fit based on project/brand alignment
    // Don't mention partnership counts or database status
    if (!evidence) {
      evidence = null; // Let AI generate a natural fit reason
    }
    ...
    if (communications?.meetings?.length > 0) {
      const relevantMeeting = communications.meetings.find(m =>
        wholeWordRegex.test(m.title || '') || wholeWordRegex.test(m.summary?.overview || '')
      );
      if (relevantMeeting) {
        evidence.source = `In the meeting "${relevantMeeting.title}", it was noted:`;
        const overview = relevantMeeting.summary?.overview || '';
        const sentences = overview.match(/[^.!?]+[.!?]+/g) || [];
        evidence.snippet = sentences.find(s => wholeWordRegex.test(s))?.trim() || 
                          "A specific mention was found, but the context could not be extracted.";
      }
    }

    // Check for actual mentions in emails
    if (!evidence.source && communications?.emails?.length > 0) {
      const relevantEmail = communications.emails.find(e =>
        wholeWordRegex.test(e.subject || '') || wholeWordRegex.test(e.preview || '')
      );
      if (relevantEmail) {
        evidence.source = `In an email from ${relevantEmail.fromName || relevantEmail.from}, it was noted:`;
        const preview = relevantEmail.preview || '';
        evidence.snippet = `...${preview}...`; // Email previews are already snippets
      }
    }
...
You will generate ideas for these brands:
...
EVIDENCE_SOURCE: ${brand.evidence.source || "No specific meeting or email mentions were found for this brand."}
EVIDENCE_SNIPPET: ${brand.evidence.snippet || "N/A"}
</BRAND_${idx + 1}>
`).join('\n')}

CRITICAL INSTRUCTIONS:
1. **Integration Idea:** Write a brief, exciting on-screen or campaign description that fits naturally into the story.

2. **Why it works:** Explain why THIS specific brand is perfect for THIS specific production. Connect the brand's category/subcategories to the project's genre, themes, target audience, or story elements. Be specific about the natural fit.

3. **HB Insights:** This is the most important rule. Follow EXACTLY:
   - If an "EVIDENCE_SNIPPET" is provided and is not "N/A", you MUST quote it. Your response for this field must be formatted like this: `[EVIDENCE_SOURCE] "[EVIDENCE_SNIPPET]"`
   - If "EVIDENCE_SOURCE" is "No specific meeting or email mentions were found for this brand.", you MUST state: "This is a fresh creative idea based on strong contextual alignment."
   - DO NOT LIE or invent insights.
   - NEVER mention partnership counts, database status, or generic phrases.
   - Be specific to THIS production and THIS brand.

4. **Formatting:** Follow the output format EXACTLY. No asterisks, no extra text.
