...
BRAND ${idx + 1}: ${brand.brand_name || brand.properties?.brand_name || brand.name || 'Unknown'}
Strategic Fit: ${brand.deepInsights?.strategicFit || brand.reason}
Integration Ideas:
${brand.deepInsights?.integrationIdeas || brand.integrationIdeas || ''}
...
Return a single JSON object with the key "pitches", containing an array of objects. Each object should have:
- "brandName": string
- "integrationIdeas": string (combine all the integration ideas into ONE paragraph)
- "whyItWorks": string
- "hbInsights": string`;
  } else {
...
1. **Integration Ideas:** Write a single paragraph (40-60 words) combining 2-3 creative integration ideas into one flowing description.
...
CRITICAL: Return ONLY a valid JSON object with a single key "pitches". The value should be an array of objects, where each object represents a brand and has the following keys:
- "brandName": string (the brand name)
- "integrationIdeas": string (a single paragraph combining 2-3 integration ideas)
- "whyItWorks": string (explanation of natural fit)
- "hbInsights": string (MUST be the exact EVIDENCE_PROVIDED text or "This is a fresh creative idea based on strong contextual alignment.")
`;
  ...
  const enrichedBrandsWithContent = topBrands.map((originalBrand, index) => {
    ...
    if (matchedPitch) {
      parsedContent = [
        ...
        `Integration: ${matchedPitch.integrationIdeas || 'Strategic brand placement'}`,
    ...
    return {
      ...
      // Add deepInsights structure for frontend
      deepInsights: matchedPitch ? {
        integrationIdeas: matchedPitch.integrationIdeas || '',
        whyItWorks: matchedPitch.whyItWorks || '',
        insight: matchedPitch.hbInsights || ''
      } : {
        integrationIdeas: originalBrand.deepInsights?.integrationIdeas || originalBrand.integrationIdeas || '',
        whyItWorks: originalBrand.deepInsights?.whyItWorks || originalBrand.whyItWorks || originalBrand.reason || '',
        insight: originalBrand.deepInsights?.insight || originalBrand.hbInsight || originalBrand.evidence || ''
      },
