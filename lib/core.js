...
export async function handleClaudeSearch(
  ...
  switch (intent.tool) {
    ...
    case 'create_pitches_for_brands': {
      ...
      // Parse brand IDs if they are numbers
      const processedBrands = [];
      for (const brandNameOrId of brands) {
        // Check if it's a numeric ID
        if (/^\d+$/.test(brandNameOrId)) {
          // This is an ID, fetch the brand details
          add({ type: 'info', text: `🔍 Looking up brand ID ${brandNameOrId}...` });
          try {
            ...
            if (searchResult?.results?.length > 0) {
              ...
              processedBrands.push({
                id: brand.id,
                name: brand.properties.brand_name || `Brand ${brandNameOrId}`,
              ...
              add({ type: 'result', text: `✅ Found: ${brand.properties.brand_name}` });
            } else {
              // Try searching by brand name in case it's stored differently
              const nameSearchResult = await hubspotAPI.searchBrands({
                query: brandNameOrId,
                limit: 1,
                properties: ['brand_name', 'main_category', 'product_sub_category__multi_', 
                             'client_status', 'client_type', 'partnership_count', 'deals_count']
              ...
              if (nameSearchResult?.results?.length > 0) {
                const brand = nameSearchResult.results[0];
                processedBrands.push({
                  id: brand.id,
                  name: brand.properties.brand_name || brandNameOrId,
                  category: brand.properties.main_category || 'Unknown',
                  subcategories: brand.properties.product_sub_category__multi_ || '',
                  clientStatus: brand.properties.client_status || '',
                  clientType: brand.properties.client_type || '',
                ...
                // Fallback: treat as external brand
                processedBrands.push({
                  id: brandNameOrId,
                  name: `Brand ${brandNameOrId}`,
                  category: 'Unknown',
                  isExternal: true
                });
              }
            ...
            console.error(`[handleClaudeSearch] Error fetching brand ${brandNameOrId}:`, error);
            processedBrands.push({
              id: brandNameOrId,
              name: `Brand ${brandNameOrId}`,
              category: 'Unknown',
          ...
          // This is already a brand name - search for it in HubSpot
          add({ type: 'info', text: `🔍 Searching for ${brandNameOrId}...` });
          try {
            const searchResult = await hubspotAPI.searchBrands({
              query: brandNameOrId,
              limit: 1,
            ...
            if (searchResult?.results?.length > 0) {
              ...
              add({ type: 'result', text: `✅ Found: ${brand.properties.brand_name} in HubSpot` });
            } else {
              // Brand not in HubSpot - treat as external
              processedBrands.push({
                id: `external-${brandNameOrId.replace(/\s+/g, '-').toLowerCase()}`,
                name: brandNameOrId,
                category: 'Unknown',
                isExternal: true
              });
              add({ type: 'info', text: `ℹ️ ${brandNameOrId} not found in HubSpot, treating as external brand` });
            }
          } catch (error) {
            console.error(`[handleClaudeSearch] Error searching for ${brandNameOrId}:`, error);
            processedBrands.push({
              id: `error-${brandNameOrId.replace(/\s+/g, '-').toLowerCase()}`,
              name: brandNameOrId,
              category: 'Unknown',
              error: true
            });
      ...
      add({ type: 'start', text: `🧠 Generating pitch structure for: ${processedBrands.map(b => b.name).join(', ')}` });
    
      // Generate creative content for these brands
      const creativeResult = await generateCreativeContent(processedBrands, {
        add,
        actualProjectName: knownProjectName || 'your project',
        search_term: userMessage,
        partnershipData: {
      ...
      return {
        ...
        finalReplyText: creativeResult.finalReplyText,
        organizedData: {
          dataType: 'BRAND_PITCHES',
          detailedBrands: creativeResult.enrichedBrands,
          brandSuggestions: [],
          projectName: knownProjectName,
