...
async function generateIntelligentSearchTerms(partnershipData, { add }) {
  // If no AI key or synopsis, fall back to simple terms
  if (!openAIApiKey || !partnershipData.synopsis) {
    const fallbackTerms = [];
    if (partnershipData.title) fallbackTerms.push(partnershipData.title);
    if (partnershipData.cast && partnershipData.cast[0]) fallbackTerms.push(partnershipData.cast[0]);
    return fallbackTerms.slice(0, 3);
  }
  
  try {
    const prompt = `You are a senior brand strategist with deep expertise in entertainment. Your task is to analyze a project synopsis and generate a JSON object containing a concise list of 3 to 5 highly effective search terms to find relevant brand conversations in a meeting database.

<RULES_FOR_EFFECTIVE_TERMS>
1. **Primary Identifiers:** Always include the main project title and the lead actor's name. These are the most direct signals.
2. **Specific, Actionable Themes:** Extract 1-2 core, tangible themes, objects, or settings from the plot that a brand might realistically be discussed alongside. Think "street racing" or "high-end streetwear," NOT generic genres like "Action."
3. **Smart Category Inference:** Based on the plot, identify 1-2 niche but relevant business categories. Think one level deeper than the obvious. For a story about a slick operator in Atlanta, "bespoke menswear" or "luxury watches" is much better than just "Fashion."
</RULES_FOR_EFFECTIVE_TERMS>

**Project Title:** ${partnershipData.title || 'Unknown'}
**Cast:** ${Array.isArray(partnershipData.cast) ? partnershipData.cast.join(', ') : (partnershipData.cast || 'Unknown')}
...
**Output Instructions:**
- Return ONLY a valid JSON object in the format: {"search_terms": ["Term 1", "Term 2", ...]}
- The list should contain a maximum of 5 terms.`;
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: MODELS.openai.chatMini,
        messages: [
          { role: 'system', content: 'You are a brand strategy expert. Return only valid JSON with search_terms array.' },
          { role: 'user', content: prompt }
        ],
        temperature: 0.3,
        max_tokens: 200
    ...
    if (result && result.search_terms && Array.isArray(result.search_terms)) {
      const terms = result.search_terms.slice(0, 5);  // Ensure max 5 terms
      add({ type: 'info', text: `🎯 Generated intelligent search terms: ${terms.join(', ')}` });
      return terms;
    }
...
async function searchCommunicationsForClues(partnershipData, { add }) {
  // Use the new intelligent search term generator for a single, smart search
  const primarySearchTerms = await generateIntelligentSearchTerms(partnershipData, { add })
