...
function generateReason(brand, partnershipData) {
  ...
  if (category.includes('fashion') && (partnershipGenres.includes('drama') || partnershipGenres.includes('romance'))) 
    return 'Perfect for character wardrobe and building authentic style.';
  if (category.includes('technology') && partnershipGenres.includes('sci-fi')) 
    return 'Ideal for showcasing future-tech as a key narrative device.';
  ...
  if (category.includes('energy drink') && partnershipGenres.includes('action'))
    return 'High-energy brand perfect for adrenaline-fueled sequences.';
  if (category.includes('jewelry') && partnershipGenres.includes('romance'))
    return 'Symbolic placement opportunities for romantic moments.';
  if (category.includes('gaming') && (partnershipGenres.includes('sci-fi') || partnershipGenres.includes('action')))
    return 'Gaming integration aligns with tech-savvy or young adult audience.';

  // Use partnership count/deals as secondary criteria
  const partnershipCount = parseInt(brand.partnership_count || 0);
  const dealsCount = parseInt(brand.deals_count || 0);
  ...
  // Use the bucket rationale as a fallback
  if (brand.sourceBucket === '🔥 Active Client') 
    return 'Currently engaged - easiest to activate for a quick win.';
  if (brand.sourceBucket === '🎯 Category Match') 
    return 'Fresh prospect that fits the project genre perfectly.';
  if (brand.sourceBucket === '📈 Win-Back') 
    return 'Previous relationship with proven success, ready to rekindle.';

  // Default fallback
  return 'Strong potential match based on creative alignment.';
}
...
async function fetchCuratedBrandBuckets(partnershipData, { add }) {
  ...
  const commsTerms = uniqShort([partnershipData.title, ...(partnershipData.cast || [])].filter(Boolean), 7);
  ...
  // Add communications insight summary
  if (communications.meetings.length > 0 || communications.emails.length > 0) {
    add({ type: 'result', text: `📊 Found ${communications.meetings.length} meetings and ${communications.emails.length} emails for context` });
  }
