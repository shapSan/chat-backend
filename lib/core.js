...
/**
 * Simple AI brand picker based on creative fit - picks brands that match the genre
 */
...
{"brands": ["Brand1", "Brand2", ...]}`;
  ...
  try {
    ...
    console.log(`[getCreativeFitBrands] AI selected ${preFilteredBrands.length} brands`);
    ...
    try {
      ...
      console.log(`[getCreativeFitBrands] Cached ${preFilteredBrands.length} brands for "${projectName}"`);
    } catch (error) {
      console.error('[getCreativeFitBrands] Failed to cache results:', error.message);
    }
