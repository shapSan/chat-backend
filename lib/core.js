...
async function searchCommunicationsForClues(partnershipData, { add }) {
  ...
  if (primarySearchTerms.length > 0) {
    ...
    for (const term of primarySearchTerms) {
      ...
      const wholeWordRegex = new RegExp(`\\b${term.replace(/[.*+?^${}()|[\]\\]/g, '\\      if (ffResult.status === 'fulfilled' && ffResult.value.transcripts) {
      ...
      if (ffResult.status === 'fulfilled' && ffResult.value.transcripts) {
        ffResult.value.transcripts.forEach(t => {
          ...
          // Only add if term appears as whole word
 ...
 */
// Patch file for updating generateCreativeContent function in core.js
// This implements the HB Insights improvements

export function patchGenerateCreativeContent(originalFunction) {
  return async function generateCreativeContent(topBrands, { add, actualProjectName, search_term, partnershipData, communications }) {
    ...
    // --- IMPROVED: Find specific evidence with context for each brand ---
    const brandsWithEvidence = topBrands.map(brand => {
      let evidence = { source: null, snippet: null }; // Change evidence to an object
      const brandNameLower = (brand.name || '').toLowerCase();
      const brandWordRegex = new RegExp(`\\b${brandNameLower.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'i');

      if (communications?.meetings?.length > 0) {
        const relevantMeeting = communications.meetings.find(m => {
          const textToSearch = `${m.title || ''} ${m.summary?.overview || ''} ${m.summary?.keywords?.join(' ') || ''}`;
          return brandWordRegex.test(textToSearch);
        });
        ...
        if (relevantMeeting) {
          ...
          evidence.snippet = sentences.find(s => brandWordRegex.test(s))?.trim() || "A specific mention was found, but the context could not be extracted.";
        }
      }
    ...
    // Updated AI prompt with evidence structure
    const creativePrompt = `You are an expert Hollywood brand integration strategist. Your task is to create two natural, budget-realistic partnership pitches for "${actualProjectName}".
...
${brandsWithEvidence.map((brand, idx) => `
<BRAND_${idx + 1}>
Name: ${brand.name}
...
1. **Why it works:** Your reasoning MUST deeply connect the brand's specific Category and Subcategories to the project's Genre, Vibe, and Synopsis. Be specific.
2. **HB Insights:** This is the most important rule.
...
3. **Format:** Follow the output format EXACTLY. No extra text, no summaries, no asterisks.

<OUTPUT_FORMAT>
Brand: [Brand Name]
Integration: [A creative, specific, and exciting on-screen integration idea.]
Why it works: [Your reasoning for the natural fit based on Category/Genre connection.]
HB Insights: [Your insight based on the EVIDENCE_PROVIDED rules above.]
</OUTPUT_FORMAT>`;
...
HB Insights: ${evidence.source && evidence.snippet ? `${evidence.source} "${evidence.snippet}"` : 'This is a fresh creative idea based on strong contextual alignment.'}`;
      }).join('\n\n');
    }
