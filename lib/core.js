...
import { logStage, HB_KEYS } from './hbDebug.js';
...
// Export the renamed imports
export { 
  utilsProgressInit as progressInit, 
  utilsProgressPush as progressPush, 
  utilsProgressDone as progressDone,
  utilsProgKey as progKey
};
 ...
 * @example isCategoryMatch("Beverage", "drinks") → true
 * @example isCategoryMatch("Automotive", "cars") → true
 */
...
Select exactly 30 brands. Make reasons specific and unique.`;
  ...
  try {
    ...
    // hbDebug: Log AI-selected brands
    logStage(HB_KEYS.AI_BRAND_SELECTION, {
      projectName: projectContext.title,
      totalBrandsAnalyzed: brandsToAnalyze.length,
      selectedCount: finalBrands.length,
      topBrands: finalBrands.slice(0, 5).map(b => ({
        name: b.brand_name,
...
async function runQuickMatchOrchestrator(
  intent,
  { add, knownProjectName, userMessage, mcpThinking, conversationContext, knownPartnershipData, lastProductionContext, selectedBrands }
) {
  // Check if selectedBrands provided - if so, skip AI selection
  if (selectedBrands && Array.isArray(selectedBrands) && selectedBrands.length > 0) {
    add({ type: 'info', text: `🎯 Using ${selectedBrands.length} user-selected brands (skipping AI selection)` });
    
    // Use the resolved partnership data directly from handleClaudeSearch
    const partnershipData = knownPartnershipData;
    const projectName = partnershipData?.title || partnershipData?.partnership_name || 'project';
    
    // FETCH FULL BRAND DATA from HubSpot for each selected brand
    const normalizedBrands = [];
    for (const selectedBrand of selectedBrands) {
      const brandId = selectedBrand.id;
      if (!brandId) {
        console.warn('[QuickMatch] Skipping brand without ID:', selectedBrand);
        continue;
      }
      
      try {
        // Fetch full brand data from HubSpot by ID
        const fullBrandData = await hubspotAPI.getObjectById('companies', brandId, [
        ...
        if (fullBrandData) {
          // Use the FULL HubSpot data (with properties object intact)
          normalizedBrands.push(fullBrandData);
          add({ type: 'info', text: `✅ Fetched full data for ${fullBrandData.properties?.brand_name || selectedBrand.name}` });
        } else {
          console.warn('[QuickMatch] Could not fetch brand data for ID:', brandId);
        }
      } catch (error) {
        console.error('[QuickMatch] Error fetching brand:', brandId, error.message);
      }
    }
    
    // Use the selected brands directly
    add({ type: 'process', text: '⚡ Quick Match: Generating creative pitches from selected brands...' });
    
    const creativeResult = await generateCreativeContent(normalizedBrands.slice(0, 2), {
      add,
    ...
    return prepareFrontendPayload({
      ...
      allRankedBrands: normalizedBrands,
      mcpThinking,
  ...
  if (!partnershipData) {
    add({ type: 'warning', text: '⚠️ No partnership data provided.' });
...
export async function runBrandSearchOrchestrator(
  ...
  // Check if selectedBrands provided - if so, run FULL deep dive on those brands
  if (selectedBrands && Array.isArray(selectedBrands) && selectedBrands.length > 0) {
    add({ type: 'info', text: `🔬 Running FULL deep dive on ${selectedBrands.length} user-selected brands` });
    
    console.log('[runBrandSearchOrchestrator] selectedBrands received:', selectedBrands.map(b => ({
      id: b.id,
    ...
    console.log('[runBrandSearchOrchestrator] Partnership data:', {
      hasData: !!partnershipData,
      title: partnershipData?.title,
      hasSynopsis: !!partnershipData?.synopsis,
      synopsisLength: partnershipData?.synopsis?.length
    });
    ...
    for (const selectedBrand of selectedBrands) {
      const brandId = selectedBrand.id;
      if (!brandId) {
        console.warn('[FullMatch] Skipping brand without ID:', selectedBrand);
        continue;
      }
      ...
      try {
        ...
        if (fullBrandData) {
          ...
          console.warn('[FullMatch] Could not fetch brand data for ID:', brandId);
        }
      } catch (error) {
        console.error('[FullMatch] Error fetching brand:', brandId, error.message);
      }
    }
    
    // Run FULL deep dive on selected brands (no light mode)
    ...
    const enhancedBrands = normalizedBrands.map((brand, idx) => {
    ...
    const creativeResult = await generateCreativeContent(enhancedBrands.slice(0, 2), {
      add,
      actualProjectName: partnershipData.title || partnershipData.partnership_name || projectName,
      search_term: userMessage,
      partnershipData,
    ...
    return prepareFrontendPayload({
      ...
      partnershipData,
      communications: { meetings: [], emails: [] },
      finalReplyText: creativeResult.finalReplyText || `Generated integration ideas for selected brands for ${partnershipData.title || partnershipData.partnership_name || projectName}`
    });
  }
  ...
  if (!partnershipData) {
    ...
    return {
      ...
      organizedData: {
        ...
        communications: { meetings: [], emails: [] },
        error: 'No partnership data available'
      },
    };
  }
  ...
  if (requestedCategory) {
    ...
    add({ type: 'info', text: `🏷️ Looking for ${requestedCategory} brands...` });
  }
  if (brandCount) {
...
// SMART PROJECT NAME RESOLUTION
  // Only extract from search_term if we have NO project name at all
  if (!projectName && intent?.args?.search_term) {
    const searchTerm = intent.args.search_term;
    
    // Skip extraction if it's a generic reference like "this project", "that production"
    const isGenericReference = /^(this|that|the|current|my|our)\s+(project|production|show|film|movie)$/i.test(searchTerm.trim());
    
    if (isGenericReference) {
      // This is a follow-up - user is referencing the current project
      console.log('[runBrandSearchOrchestrator] Generic reference detected, treating as follow-up');
      // projectName stays null, will be resolved from context below
    } else if (searchTerm && searchTerm.length > 0) {
      // Try to extract project name from search term
      ...
      // Only use cleaned if it's significantly shorter (likely just the project name)
...
DO NOT extract cast information - we only use verified cast data from our database.`;
      ...
      if (response.ok) {
        ...
        if (extracted.title) {
          ...
          console.log('[runBrandSearchOrchestrator] AI extracted (no cast):', extracted);
        }
      }
    } catch (e) {
  ...
  // Build final partnership data structure
  ...
  // CRITICAL FIX: Build final partnership data with exact same pattern for all fields
  const finalPartnershipData = {
      title: partnershipData?.title || projectName || extractedData.title,
      ...
      location: partnershipData?.location || partnershipData?.shoot_location__city_ || null,
      shoot_location__city_: partnershipData?.shoot_location__city_ || null,
      // Cast - handled exactly like other fields
      cast: partnershipData?.cast || partnershipData?.main_cast || null,
      main_cast: partnershipData?.main_cast || partnershipData?.cast || null,
      vibe: partnershipData?.vibe || partnershipData?.genre_production || extractedData.genre || extractedData.vibe || null,
      genre_production: partnershipData?.genre_production || partnershipData?.vibe || null,
    ...
    if (requestedCategory && ranked?.length) {
      ...
      if (filteredRanked.length > 0) {
        ...
        if (smartFiltered.length > 0) {
          ...
          add({ type: 'info', text: `🎯 Found ${ranked.length} ${requestedCategory} brands (smart match)` });
        } else {
...
async function generateCreativeContent(topBrands, { add, actualProjectName, search_term, partnershipData, communications }) {
  ...
  console.log('[generateCreativeContent] Called with:', {
    ...
    hasPartnershipData: !!partnershipData,
    partnershipTitle: partnershipData?.title,
    actualProjectName,
    brandsHaveDeepInsights: topBrands.some(b => b.deepInsights)
  });
...
If from MCP data, include specific REAL meeting references like "Meeting: [name] on [date]" or "Email: [subject]". DO NOT LIE IF YOU DONT HAVE PROOF OF INSIGHT!`;
  ...
  if (!finalFormattedText) {
    console.log('[generateCreativeContent] AI did not return content, using fallback');
...
HB Insights: ${partnershipCount} previous partnerships and ${dealsCount} deals demonstrate proven entertainment value.`;
    ...
    console.log('[generateCreativeContent] AI returned content, length:', finalFormattedText.length);
  }
  ...
  // hbDebug: Log enriched brand data with creative content
  logStage(HB_KEYS.CREATIVE_GENERATION, {
    projectName: actualProjectName,
    brandCount: enrichedBrandsWithContent.length,
    brands: enrichedBrandsWithContent.map(b => ({
      name: b.brand_name || b.properties?.brand_name,
      hasContent: !!b.content,
      contentLength: b.content?.length || 0,
      hasEvidence: !!b.evidence,
      hasDeepInsights: !!b.deepInsights,
      insightContext: b.insightContext || null
    }))
  });
...
function prepareFrontendPayload({
  ...
  // DIAGNOSTIC LOGGING: Log sample brand structure
  if (detailedBrands.length > 0) {
    console.log('[prepareFrontendPayload] SAMPLE detailedBrands[0] structure:', {
      id: detailedBrands[0].id,
      name: detailedBrands[0].name,
      brand_name: detailedBrands[0].brand_name,
      hasProperties: !!detailedBrands[0].properties,
      propertiesKeys: detailedBrands[0].properties ? Object.keys(detailedBrands[0].properties) : [],
      topLevelKeys: Object.keys(detailedBrands[0]).filter(k => k !== 'properties'),
      main_category: detailedBrands[0].main_category,
      client_type: detailedBrands[0].client_type,
      partnership_count: detailedBrands[0].partnership_count,
    });
  }
  if (brandSuggestions.length > 0) {
    console.log('[prepareFrontendPayload] SAMPLE brandSuggestions[0] structure:', {
      id: brandSuggestions[0].id,
      name: brandSuggestions[0].name,
      brand_name: brandSuggestions[0].brand_name,
      hasProperties: !!brandSuggestions[0].properties,
      propertiesKeys: brandSuggestions[0].properties ? Object.keys(brandSuggestions[0].properties) : [],
      topLevelKeys: Object.keys(brandSuggestions[0]).filter(k => k !== 'properties'),
      main_category: brandSuggestions[0].main_category,
      client_type: brandSuggestions[0].client_type,
      partnership_count: brandSuggestions[0].partnership_count,
    });
  }
  
  // hbDebug: Log final frontend payload structure
  logStage(HB_KEYS.FRONTEND_PAYLOAD, {
    projectName,
    detailedBrandsCount: detailedBrands.length,
    brandSuggestionsCount: brandSuggestions.length,
    hasCommunications: !!(communications && (communications.meetings?.length || communications.emails?.length)),
    communicationsStats: {
      meetings: communications?.meetings?.length || 0,
      emails: communications?.emails?.length || 0
    },
    partnershipDataFields: Object.keys(partnershipData || {}).filter(k => partnershipData[k] != null),
    detailedBrandsPreview: detailedBrands.slice(0, 3).map(b => ({
      id: b.id,
      name: b.name,
      category: b.main_category,
      hasContent: !!b.content,
...
export async function handleClaudeSearch(
  ...
  knownProjectName,  // Keep for potential fallback ONLY during resolution
  runId,
  onStep = () => {},
  knownPartnershipData,  // Partnership data ONLY if from frontend panel
  selectedBrands  // User-selected brands from frontend
) {
  ...
  try {
    ...
    add({ type: 'process', text: 'Understanding request...' });
    const intent = await routeUserIntent(userMessage, conversationContext, lastProductionContext);
    if (!intent) {
      // Should default to answer_general_question, but handle null just in case
      console.error('[handleClaudeSearch] Intent routing failed unexpectedly.');
      return null; // Let the caller handle fallback to general AI answer
    }
    const routeText = `🧭 Route: ${intent.tool}${intent.args ? ` (${Object.entries(intent.args).map(([k,v]) => `${k}=${v}`).join(', ')})` : ''}`;
    add({ type: 'route', text: routeText });
    console.log(`[handleClaudeSearch] Intent resolved: ${intent.tool}`, intent.args || {});

    // === STEP 2: RESOLVE PROJECT CONTEXT (Name & Data) ===
    let resolvedProjectName = null;
    let resolvedPartnershipData = null;
    let contextSource = 'None'; // For logging

    // Priority 0: If intent router extracted a specific project name, try that FIRST
    // This prevents detectNewProject from misidentifying queries like "Quick Match for X"
    const intentProjectName = intent.args?.project_name || intent.args?.search_term || intent.args?.production;
    if (intentProjectName && intentProjectName.toLowerCase() !== 'new project' && !knownPartnershipData) {
      resolvedProjectName = intentProjectName;
      contextSource = 'Intent Router';
      add({ type: 'info', text: `🎯 Intent router identified project: "${resolvedProjectName}". Fetching details...` });
      if (hubspotAPI?.getPartnershipForProject) {
        const rawData = await hubspotAPI.getPartnershipForProject(resolvedProjectName);
        if (rawData) {
          resolvedPartnershipData = normalizePartnership(rawData);
          await saveSessionContext(sessionId, { projectName: resolvedProjectName, partnershipData: resolvedPartnershipData });
          add({ type: 'result', text: `✅ Fetched data for "${resolvedProjectName}" from HubSpot.` });
          contextSource += ' (Fetched)';
        } else {
          add({ type: 'warning', text: `⚠️ Project "${resolvedProjectName}" not found in HubSpot. Using minimal context.` });
          resolvedPartnershipData = { title: resolvedProjectName, partnership_name: resolvedProjectName, name: resolvedProjectName, synopsis: userMessage };
          await saveSessionContext(sessionId, { projectName: resolvedProjectName, partnershipData: resolvedPartnershipData });
        ...
        resolvedPartnershipData = { title: resolvedProjectName, partnership_name: resolvedProjectName, name: resolvedProjectName, synopsis: userMessage };
        await saveSessionContext(sessionId, { projectName: resolvedProjectName, partnershipData: resolvedPartnershipData });
        contextSource += ' (Minimal - No API)';
      }
    }
    // Priority 1: Data provided by the frontend panel
    else if (knownPartnershipData) {
      resolvedPartnershipData = normalizePartnership(knownPartnershipData); // Ensure it's normalized
      resolvedProjectName = await extractProjectName(resolvedPartnershipData) || 'Project from Panel';
      contextSource = 'Panel';
      add({ type: 'info', text: `📦 Using provided partnership data for "${resolvedProjectName}"` });
      // Save/update session immediately if panel provides data
      await saveSessionContext(sessionId, {
        projectName: resolvedProjectName,
        partnershipData: resolvedPartnershipData
      });
    } else {
      // Priority 2: Check for Explicit New Project detection
      ...
      if (newProjectInfo && newProjectInfo.isNewProject && newProjectInfo.title) {
        resolvedProjectName = newProjectInfo.title;
        contextSource = 'New Project Detected';
        add({ type: 'info', text: `✨ New project detected: "${resolvedProjectName}". Fetching details...` });
        ...
        if (hubspotAPI?.getPartnershipForProject) {
          ...
          if (rawData) {
            resolvedPartnershipData = normalizePartnership(rawData);
            await saveSessionContext(sessionId, { projectName: resolvedProjectName, partnershipData: resolvedPartnershipData });
            add({ type: 'result', text: `✅ Fetched data for new project "${resolvedProjectName}".` });
            contextSource += ' (Fetched)';
          } else {
            add({ type: 'warning', text: `⚠️ New project "${resolvedProjectName}" not found in HubSpot. Using minimal context.` });
            resolvedPartnershipData = { title: resolvedProjectName, partnership_name: resolvedProjectName, name: resolvedProjectName, synopsis: newProjectInfo.synopsis || userMessage };
            // Save minimal context to session
            await saveSessionContext(sessionId, { projectName: resolvedProjectName, partnershipData: resolvedPartnershipData });
            contextSource += ' (Minimal)';
          }
        } else {
        ...
        // Priority 3: Load from existing Session
        const sessionContext = await loadSessionContext(sessionId);
        if (sessionContext?.projectName && sessionContext?.partnershipData) {
          resolvedProjectName = sessionContext.projectName;
          resolvedPartnershipData = sessionContext.partnershipData; // Already normalized during save
          contextSource = 'Session';
          add({ type: 'info', text: `🔄 Resuming session for project: "${resolvedProjectName}"` });
        } else {
          // Priority 4: Use project name from Intent Router args
          const intentProjectName = intent.args?.project_name || intent.args?.search_term || intent.args?.production;
          if (intentProjectName && intentProjectName.toLowerCase() !== 'new project') {
            resolvedProjectName = intentProjectName;
            contextSource = 'Intent Args';
            add({ type: 'info', text: `🎯 Project identified from intent: "${resolvedProjectName}". Fetching details...` });
            if (hubspotAPI?.getPartnershipForProject) {
              const rawData = await hubspotAPI.getPartnershipForProject(resolvedProjectName);
              if (rawData) {
                resolvedPartnershipData = normalizePartnership(rawData);
                await saveSessionContext(sessionId, { projectName: resolvedProjectName, partnershipData: resolvedPartnershipData });
                add({ type: 'result', text: `✅ Fetched data for "${resolvedProjectName}".` });
                contextSource += ' (Fetched)';
              } else {
                add({ type: 'warning', text: `⚠️ Project "${resolvedProjectName}" not found in HubSpot. Using minimal context.` });
                resolvedPartnershipData = { title: resolvedProjectName, partnership_name: resolvedProjectName, name: resolvedProjectName, synopsis: userMessage };
            ...
            // Priority 5: Fallback - Try knownProjectName (from older logic/migration) or extract from history
            const historyProjectName = lastProductionContext?.match(/\[PRODUCTION:([^\]]+)\]/i)?.[1]?.trim();
            resolvedProjectName = knownProjectName || historyProjectName || null;
            if (resolvedProjectName) {
              contextSource = knownProjectName ? 'Fallback Prop' : 'History Extract';
              add({ type: 'info', text: `⚠️ Using fallback project name: "${resolvedProjectName}" (${contextSource}). Fetching details...` });
              // Attempt fetch and save as above
              if (hubspotAPI?.getPartnershipForProject) {
                const rawData = await hubspotAPI.getPartnershipForProject(resolvedProjectName);
                if (rawData) {
                  resolvedPartnershipData = normalizePartnership(rawData);
              ...
              // No project context could be determined
              contextSource = 'Undetermined';
              add({ type: 'warning', text: '⚠️ Could not determine project context.' });
              resolvedPartnershipData = { title: "Unknown Project", synopsis: userMessage }; // Minimal default
            }
          }
        }
      }
    }
    console.log(`[handleClaudeSearch] Context resolved via: ${contextSource}. Project: "${resolvedProjectName}"`);
    ...
    // === STEP 3: EXECUTE INTENT WITH RESOLVED CONTEXT ===
    switch (intent.tool) {
      case 'quick_match_brands': {
        return await runQuickMatchOrchestrator(intent, {
          ...
          conversationContext,
          lastProductionContext,
          knownPartnershipData: resolvedPartnershipData, // Pass resolved data
          selectedBrands
        });
      }
    ...
    case 'get_brand_activity': {
      ...
      case 'create_pitches_for_brands': {
        const brandsFromIntent = intent.args.brand_names || [];
        add({ type: 'start', text: `🧠 Generating pitches for ${brandsFromIntent.length} brands...` });
        
        // ***** FIX: Determine if we have real brand objects or just names *****
        // If selectedBrands is provided from the frontend (e.g., PartnershipsPanel),
        // it contains brand objects with IDs - use those.
        // If we only have brand names from text input (e.g., BrandsPanel),
        // pass null so runBrandSearchOrchestrator uses its userSuggestedBrands logic
        // which includes the HubSpot fallback search.
        const useOriginalSelectedBrands = selectedBrands && selectedBrands.length > 0;
        
        return await runBrandSearchOrchestrator(intent, { // REUSE orchestrator
          add,
          userMessage, // Orchestrator needs this to extract userSuggestedBrands
          mcpThinking,
          conversationContext,
          lastProductionContext,
          knownPartnershipData: resolvedPartnershipData, // Pass resolved data
          // CRITICAL: Only pass selectedBrands if they're real objects with IDs.
          // Otherwise pass null to trigger userSuggestedBrands extraction + HubSpot fallback
          selectedBrands: useOriginalSelectedBrands ? selectedBrands : null
        });
      }
