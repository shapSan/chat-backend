  ...
  try {
    switch (intent.tool) {
      case 'find_brands': {
        ...
        // COMPLETE STRUCTURED RESPONSE FOR FRONTEND
        // Step 1: Prepare the full list for the picker (minimal details)
        ...
        // Step 2: Get top 2 brands for detailed cards
        const top2Brands = finalBrands.slice(0, 2);
        
        // Step 3: Create parsed brand objects for the frontend cards
        // The main handler will call getTextResponseFromClaude with these 2 brands
        // and then parse the response to create this structure
        const parsedBrands = top2Brands.map(brand => ({
          name: brand.name,
          content: [
            {
              type: 'integration',
              text: `${brand.category} integration perfectly aligned with production needs`
            },
            {
              type: 'whyItWorks', 
              text: brand.reason || `Strong fit based on ${brand.partnershipCount} partnerships and proven track record`
            },
            {
              type: 'hbInsights',
              text: brand.insight?.rawText || `Active client with ${brand.dealsCount} deals in pipeline`
            }
          ],
          // Include raw brand data for main handler to enhance
          _brandData: brand,
          _needsAIGeneration: true  // Flag for main handler to generate proper content
        }));
        
        // Step 4: Extract or determine project name
        const projectName = knownProjectName || 
                          extractedTitle || 
                          (search_term.length > 50 ? 'Brand Recommendations' : search_term.substring(0, 50));
        
        add({ type: 'process', text: `📦 Complete JSON ready: ${top2Brands.length} detailed brands, ${brandSuggestionsForPicker.length} picker options` });

        return {
          mcpThinking,
          organizedData: {
            // Step 5: Main trigger flag for frontend
            dataType: 'BRAND_RECOMMENDATIONS',
            
            // For the picker: full list with minimal details
            brandSuggestions: brandSuggestionsForPicker,
            
            // For the cards: top 2 brands with parsed content structure
            detailedBrands: parsedBrands,
            
            // Project name for the title
            projectName: projectName,
            
            // Additional context for the main handler to use when calling AI
            _aiContext: {
              synopsis: search_term,
              genre: genre,
              top2BrandsRaw: top2Brands,
              meetingsContext: supportingContext.meetings.slice(0, 3),
              emailsContext: supportingContext.emails.slice(0, 3)
            }
          },
        };
      }
