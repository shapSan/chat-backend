...
export async function runBrandSearchOrchestrator(
  ...
  // Step 1: Use AI to extract project structure from the message
  ...
  // If we don't have a clear project name, use AI to extract it
  if (!projectName && userMessage && openAIApiKey) {
    try {
...
Return format:
{
  ...
  const boostedBrands = finalPickerList.map(brand => {
    ...
    if (brandName && brandName.length > 2 && commsText.includes(brandName)) {
      ...
      add({ type: 'info', text: `💬 Signal boost for ${brand.brand_name} - found in recent communications` });
    }
...
HB Insights: ${partnershipCount} previous partnerships and ${dealsCount} deals demonstrate proven entertainment value.`;
  ...
  const enrichedBrandsWithContent = topBrands.map(originalBrand => {
    ...
    const actualBrandName = originalBrand.brand_name || originalBrand.properties?.brand_name || '';
