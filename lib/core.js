  ...
  try {
    ...
    if (newProjectInfo && newProjectInfo.isNewProject && newProjectInfo.title) {
      ...
      if (markerMatch && markerMatch[1]) {
        currentProjectName = markerMatch[1].trim();
        console.log('[handleClaudeSearch] Found project in history:', currentProjectName);
        // Will be fetched from HubSpot below
    ...
    if (currentProjectName && !currentProjectContext && hubspotAPI?.getPartnershipForProject) {
      ...
      try {
        ...
        if (rawPartnershipData) {
          ...
          // Check if this was recovered from history
          const wasRecoveredFromHistory = sessionContext === null && lastProductionContext;
          if (wasRecoveredFromHistory) {
