...
Return this exact JSON structure:
{
  ...
  const enhancedBrands = brandsForPitching.map((brand, idx) => {
    ...
    // Join integration ideas array into a single string for frontend
    const processedInsights = {
      ...insights,
      integrationIdeas: Array.isArray(insights.integrationIdeas) 
        ? insights.integrationIdeas.join(' ') 
        : insights.integrationIdeas
    };
    
    return {
      ...brand,
      deepInsights: processedInsights,
      // Use the AI's strategic fit as the primary reason
      reason: processedInsights.strategicFit || brand.reason,
      // Add integration ideas directly to brand (as string)
      integrationIdeas: processedInsights.integrationIdeas,
      whyItWorks: processedInsights.whyItWorks,
      hbInsight: processedInsights.insight
    };
  });
...
Integration Ideas: ${brand.deepInsights?.integrationIdeas || brand.integrationIdeas || 'Strategic brand placement'}
...
`;
  ...
  if (!finalFormattedText || finalFormattedText.trim() === '') {
    ...
    finalFormattedText = topBrands.map((brand, idx) => {
      ...
      if (brand.deepInsights?.integrationIdeas) {
        integration = brand.deepInsights.integrationIdeas;
      } else if (brand.integrationIdeas) {
        integration = brand.integrationIdeas;
  ...
  const enrichedBrandsWithContent = topBrands.map((originalBrand, index) => {
    ...
    if (matchedPitch) {
      ...
      const integration = originalBrand.deepInsights?.integrationIdeas || 
                        originalBrand.integrationIdeas || 
