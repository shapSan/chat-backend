...
\`\`\``;
  ...
  // CRITICAL FIX: Context Mismatch Detection and Correction
  // =================================================================
  const projectNameFromMessage = extractedData.title;
  if (projectNameFromMessage && knownProjectName && projectNameFromMessage.toLowerCase() !== knownProjectName.toLowerCase()) {
    add({ type: 'warning', text: `⚠️ Context Mismatch: Prioritizing "${projectNameFromMessage}" from your message over previous context "${knownProjectName}".` });
    projectName = projectNameFromMessage; // Prioritize the new project
    console.log('[runBrandSearchOrchestrator] Context mismatch detected - using project from current message:', projectName);
  } else if (projectNameFromMessage) {
    projectName = projectNameFromMessage; // Default to the extracted name if no conflict
  }
  // =================================================================
