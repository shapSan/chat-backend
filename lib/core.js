...
Example Output: {"selected_names": ["Brand A", "Brand B", ...]}`;
  ...
  try {
    ...
    const selectionResponse = await withTimeout(
      ...
      10000,  // 10 second timeout for selection
      null
    );
...
Example Output: {"Brand A": "Reason for A...", "Brand B": "Reason for B..."}`;
    ...
    const reasonResponse = await withTimeout(
      ...
      10000,  // 10 second timeout for reasons
      null
    );
...
export async function handleClaudeSearch(
  ...
  switch (intent.tool) {
    ...
    case 'create_pitches_for_brands': {
      ...
      // Get partnership data - use provided data first, then fallback to fetching
      let partnershipData = null;
      
      // NEW CONDITIONAL LOGIC
      if (knownPartnershipData && (knownPartnershipData.synopsis || knownPartnershipData.title)) {
        // If frontend sent complete data, use it directly and skip the fetch
        partnershipData = normalizePartnership(knownPartnershipData); // Ensure fields are consistent
        add({ type: 'info', text: `✅ Using project context provided by client.` });
        console.log('[handleClaudeSearch] Skipping HubSpot fetch, using provided partnership data.');
      } else if (projectContext && hubspotAPI?.getPartnershipForProject) {
        // Fallback to the old method if no data was passed from client
        try {
          add({ type: 'process', text: `🔍 Fetching fresh project data for "${projectContext}"...` });
