...
async function runQuickMatchOrchestrator(
  ...
  if (selectedBrands && Array.isArray(selectedBrands) && selectedBrands.length > 0) {
    ...
    // Normalize selected brands to match expected format
    const normalizedBrands = selectedBrands.map(b => ({
      ...b,
      brand_name: b.name || b.brand_name || 'Unknown',
      main_category: b.category || b.main_category || 'General',
      id: b.id || `brand-${b.name?.toLowerCase().replace(/\s+/g, '-')}`
    }));
...
export async function runBrandSearchOrchestrator(
  ...
  if (selectedBrands && Array.isArray(selectedBrands) && selectedBrands.length > 0) {
    ...
    const normalizedBrands = selectedBrands.map(b => ({
      ...
      brand_name: b.name || b.brand_name || 'Unknown',
      main_category: b.category || b.main_category || 'General',
      id: b.id || `brand-${(b.name || b.brand_name || 'unknown').toLowerCase().replace(/\s+/g, '-')}`
    }));
