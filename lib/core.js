  ...
  try {
    switch (intent.tool) {
      case 'find_brands': {
        ...
        // Full list for picker (minimal details) - ensure string IDs
        const brandSuggestionsForPicker = finalBrands.map((brand, idx) => {
          // Ensure consistent ID format - always use string IDs
          let brandId;
          if (brand.id) {
            // Convert to string (handles both numeric HubSpot IDs and wildcard IDs)
            brandId = String(brand.id);
          } else {
            // Generate fallback ID using brand name - fix special characters
            const cleanBrandName = (brand.name || `brand-${idx}`)
              .toLowerCase()
              .replace(/[^a-z0-9]+/g, '-') // Replace multiple non-alphanumeric chars with single dash
              .replace(/^-+|-+$/g, ''); // Remove leading/trailing dashes
            brandId = cleanBrandName ? `${cleanBrandName}-${idx}` : `brand-${idx}`;
          }
          
          return {
            id: brandId, // Always a string ID
            name: brand.name,
...
Make the content creative, specific, and compelling. Focus on storytelling opportunities and marketing potential.`;
        ...
        if (creativeTextResponse && creativeTextResponse.length > 50) {
          ...
          parsedBrands = brandSections.slice(0, 2).map((section, idx) => {
            ...
            // Ensure consistent ID format - always use string IDs
            let brandId;
            if (originalBrandData?.id) {
              // Convert numeric HubSpot IDs to strings
              brandId = String(originalBrandData.id);
            } else {
              // Generate fallback ID using brand name - fix special characters
              const cleanBrandName = (brandName || `brand-${idx}`)
                .toLowerCase()
                .replace(/[^a-z0-9]+/g, '-') // Replace multiple non-alphanumeric chars with single dash
                .replace(/^-+|-+$/g, ''); // Remove leading/trailing dashes
              brandId = cleanBrandName ? `${cleanBrandName}-${idx}` : `brand-${idx}`;
