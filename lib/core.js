...
async function runBrandSearchOrchestrator(intent, { add, knownProjectName, userMessage, mcpThinking, conversationContext }) {
  // Extract project name from the user message if not already known
  let projectName = knownProjectName;
  
  // If no project name provided, try to extract it from the message
  if (!projectName) {
    const projectMatch = userMessage.match(/(?:for|about|regarding|called|titled|project)\s+["']?([^"'\n]{3,50}?)["']?(?:\s+(?:starring|with|at|in|by|from)|[.,!?]|$)/i);
    if (projectMatch) {
      projectName = projectMatch[1].trim();
      console.log('[runBrandSearchOrchestrator] Extracted project name from message:', projectName);
    }
  }
  ...
  if (projectName && hubspotAPI?.getPartnershipForProject) {
    add({ type: 'info', text: `🔍 Searching for partnership data for: ${projectName}` });
    try {
      ...
      console.log('[runBrandSearchOrchestrator] Initial search result:', partnershipData);
      
      if (partnershipData) {
        add({ type: 'result', text: `✅ Found partnership data in HubSpot` });
        console.log('[runBrandSearchOrchestrator] Partnership data found:', partnershipData);
      } else {
        // Try alternative search - Scary Movie Reboot might be stored as "Scary Movie 6"
        const alternativeNames = [];
        if (knownProjectName.toLowerCase().includes('scary movie')) {
          alternativeNames.push('Scary Movie 6', 'Scary Movie', 'scary movie', 'Scary Movie Reboot');
        }
        // Try common variations
        alternativeNames.push(
          knownProjectName.replace(/reboot/i, '').trim(),
          knownProjectName.replace(/\s+\d+$/, '').trim(),
          knownProjectName.replace(/\s+/g, ' ').trim() // Normalize spaces
        );
        
        // Remove duplicates
        const uniqueAlternatives = [...new Set(alternativeNames)].filter(name => name && name !== knownProjectName);
        ...
        for (const altName of uniqueAlternatives) {
          ...
          if (partnershipData) {
            ...
            // Update the project name to the found variation
            knownProjectName = altName;
            break;
          }
        }
      }
