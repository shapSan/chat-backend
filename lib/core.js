...
async function extractProjectName(partnershipData) {
  ...
  // Try common field names first for speed
  const quickCheck = partnershipData.title || 
                     partnershipData.partnership_name || 
                     partnershipData.name || 
                     partnershipData.projectName;
  
  if (quickCheck && typeof quickCheck === 'string' && quickCheck.trim().length > 0) {
    // Remove any prefixes like "Quick Match for"
    const cleaned = quickCheck.replace(/^(quick match|fast match|generate.*?for)\s+/i, '').trim();
    if (cleaned && cleaned !== quickCheck) {
      console.log(`[extractProjectName] Cleaned "${quickCheck}" -> "${cleaned}"`);
      return cleaned;
    }
    return quickCheck;
  }
...
async function runQuickMatchOrchestrator(
  ...
  // STEP 2: Load the brand database from the fast cache
  ...
  // STEP 3: Use AI for fast matching (30 brands like regular search)
  add({ type: 'process', text: '🤠 Quick Match: Selecting 30 brands by creative fit...' });
  ...
  const matchPrompt = `You are a world-class brand strategist.
  Analyze this project context:
  ${projectContext}

  From this list of available brands, select the 30 with the most natural and creative fit.
  BRANDS AVAILABLE:
  ${brandList}

  Return ONLY a valid JSON object with your top 30 selections in the format:
  {
    "selections": [
      {"name": "Selected Brand Name 1", "reason": "A very short, clever reason for the fit."},
      {"name": "Selected Brand Name 2", "reason": "Another short reason."}
    ]
  }`;
  ...
  try {
    const response = await withTimeout(
      fetch('https://api.openai.com/v1/chat/completions', {
        ...
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${openAIApiKey}` },
        body: JSON.stringify({
          model: MODELS.openai.chatMini, // Use mini for speed
    ...
    if (!response || !response.ok) throw new Error('AI matching failed or timed out.');
    ...
    add({ type: 'result', text: `✅ AI picked ${selections.length} brands for quick match` });
    ...
    const matchedBrands = selections.map((sel, index) => {
    ...
    // STEP 5: Take top 2 for lite deep dive (same as regular search)
    const brandsForPitching = matchedBrands.slice(0, 2);
    
    add({ type: 'process', text: '⚡ Running LITE analysis on top 2 brands (30-day comms, no web search)' });
    
    // Run LITE deep dive on top 2 (same as regular search)
    const { deepDiveMultipleBrands } = await import('./deepDive.js');
    const deepDiveResults = await deepDiveMultipleBrands(
      brandsForPitching,
      partnershipData,
      { 
        isLite: true, // LITE mode for quick match
        add,
        communications: { meetings: [], emails: [] } // Skip comms search for speed
      }
    );
    ...
    // STEP 6: Generate creative content (same as regular search)
...
If no specific brands are mentioned, return: {"suggested_brands": []}`;
  ...
  // Prioritize current message over historical context
  // Only use knownProjectName if current message doesn't specify a project
  let projectName = intent?.args?.search_term || intent?.args?.project_name || intent?.args?.production || knownProjectName || '';
