...
async function getSemanticallyRankedBrands(partnershipData, { add }) {
  ...
  const brandsForAI = cachedBrands.map(b => ({
    ...
    // Removed: clientStatus and partnershipCount - not needed for creative matching
  }));

  // Extract keywords from synopsis for better matching
  let synopsisKeywords = [];
  if (partnershipData.synopsis && openAIApiKey) {
    try {
      const keywordResponse = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          model: MODELS.openai.chatMini,
          messages: [
            { role: 'system', content: 'Extract 5-8 key themes, objects, activities, and emotions from this synopsis. Return as JSON: {"keywords": [...]}' },
            { role: 'user', content: partnershipData.synopsis || '' }
          ],
      ...
      if (keywordResponse.ok) {
        ...
        const result = JSON.parse(keywordData.choices?.[0]?.message?.content || '{}');
        synopsisKeywords = result.keywords || [];
        add({ type: 'info', text: `🔑 Synopsis themes: ${synopsisKeywords.slice(0, 5).join(', ')}` });
      }
    } catch (e) {
      console.error('[getSemanticallyRankedBrands] Keyword extraction ERROR:', e);
      console.error('[getSemanticallyRankedBrands] Keyword extraction stack:', e.stack);
      add({ type: 'warning', text: `⚠️ Keyword extraction failed: ${e.message}` });
      // Continue without keywords rather than failing
    }
  }
...
Key Themes: ${synopsisKeywords.join(', ') || 'Not extracted'}
...
Synopsis excerpt: ${(partnershipData.synopsis || '').substring(0, 300)}...
</PRODUCTION_CONTEXT>
...
1. Rank brands 0-100 based on how their category/subcategories align with the production's themes and genre
2. Each "reason" MUST be UNIQUE, SPECIFIC, and reference:
   - A specific theme/keyword from the production
   - How the brand's category naturally fits that theme
   - A concrete scene or moment where it would appear
3. BAD reasons (DO NOT USE):
   - "Great for organic placement in social scenes"
   - "Fits [genre] genre well"
...
Return JSON with "brands" array of TOP 50:
{
  ...
  try {
    ...
    console.error('[getSemanticallyRankedBrands] FULL ERROR:', error);
    console.error('[getSemanticallyRankedBrands] Stack:', error.stack);
    add({ type: 'error', text: `❌ AI ranking failed: ${error.message}` });
