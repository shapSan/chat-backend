  ...
  try {
    ...
    case 'get_brand_activity': {
      ...
      case 'create_pitches_for_brands': {
        ...
        // ***** FIX: Use selectedBrands from function parameter (contains objects with IDs) *****
        // If selectedBrands is provided from the frontend, use it (it has brand objects with IDs)
        // Otherwise, create placeholder objects from brand names (though this path might not be ideal)
        const brandsToPass = (selectedBrands && selectedBrands.length > 0)
                              ? selectedBrands // Use the original array with objects and IDs
                              : brandsFromIntent.map(name => ({ name })); // Fallback if original is empty/missing
        
        return await runBrandSearchOrchestrator(intent, { // REUSE orchestrator
          add,
          userMessage, // Pass original message for context if needed
          mcpThinking,
          conversationContext,
          lastProductionContext,
          knownPartnershipData: resolvedPartnershipData, // Pass resolved data
          selectedBrands: brandsToPass // Pass the CORRECT array (either original objects or fallback)
        });
      }
