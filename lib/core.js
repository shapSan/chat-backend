...
import { o365API, searchFireflies } from './services.js';
        ...
        messages: [
          {
            role: 'system',
            content:
              'Suggest exactly 5 UNEXPECTED consumer brand names (NOT studios/distributors/production companies) for product placement in this production. Think outside the box - find hidden gem brands that would create surprising but perfect synergy. Focus on real consumer brands that make products/services that could appear on screen and pay for placement. Be creative and unpredictable. Return JSON: {"brands": ["Brand Name 1", "Brand Name 2", ...]}',
          },
  ...
  try {
    switch (intent.tool) {
      case 'find_brands': {
        ...
        if (firefliesApiKey || (msftTenantId && msftClientId && msftClientSecret)) {
