  ...
  try {
    switch (intent.tool) {
      case 'find_brands': {
        ...
        // ROBUST DUAL-PATH RESPONSE SYSTEM:
        // 1. Full list (~15-20 brands) goes to structuredData.brandSuggestions for the UI picker
        // 2. Top 2 brands get detailed AI write-ups and go to structuredData.detailedBrands
        // 3. The text reply will also contain the human-readable version of these 2 brands
        
        const fullBrandList = finalBrands;  // Keep all ~15-20 brands for the picker
        const top2Brands = finalBrands.slice(0, 2);  // Top 2 for detailed AI write-up
        
        // Prepare the top 2 brands with placeholders for detailed content
        // The main handler will call AI to fill in Integration, Why it works, and HB Insights
        const detailedBrands = top2Brands.map(brand => ({
          ...brand,
          // These fields will be populated by the AI in the main handler
          integration: null,  // Will contain detailed integration ideas
          whyItWorks: null,   // Will contain "Why it works" explanation
          hbInsights: null,   // Will contain HubSpot/meeting insights
          needsAIGeneration: true  // Flag for main handler to generate content
        }));
        
        add({ type: 'process', text: `📝 Robust dual response: ${top2Brands.length} brands for detailed write-up, ${fullBrandList.length} brands for picker` });

        return {
          mcpThinking,
          organizedData: {
            dataType: 'BRAND_RECOMMENDATIONS',  // CRITICAL: This identifier tells frontend to render brand cards
            brandSuggestions: fullBrandList,    // FULL LIST (~15-20 brands) for the UI picker
            detailedBrands: detailedBrands,     // TOP 2 with structure for detailed AI content
            brandsForDetailedResponse: top2Brands,  // TOP 2 for backward compatibility
            pools: {
            ...
            context: {
              ...
              synopsis: search_term,  // Include synopsis for AI context
              projectName: knownProjectName || extractedTitle || null,  // Include project name if known
            },
          },
        };
      }
