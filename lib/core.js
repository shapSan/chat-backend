...
export async function handleClaudeSearch(
  ...
  switch (intent.tool) {
    ...
    case 'create_pitches_for_brands': {
      ...
      // --- FIX 1 & 2: Robustly parse brands and establish project context ---
      let brands = [];
      let projectContext = knownProjectName; // Start with the reliable context from the frontend
      ...
      if (brandString) {
        ...
        // Fallback if the regex doesn't match, use what the router gave us
        brands = intent.args.brand_names || [];
      }

      // If the user specifies "for [New Project]", let it override the known context.
      const projectOverrideMatch = userMessage.match(/\s+for\s+([\w\s]+?)$/i);
      if (projectOverrideMatch && projectOverrideMatch[1].trim() !== projectContext) {
          const newProject = projectOverrideMatch[1].trim();
          // Check that it's not just a fragment of the old synopsis
          if (newProject.length < 100) {
              projectContext = newProject;
              add({ type: 'info', text: `📝 Project context switched to: "${projectContext}"` });
          }
      }

      // If after all that we still don't have a project context, try the last production context
      if (!projectContext && lastProductionContext) {
        ...
        if (projectMatch && projectMatch[1]) {
          ...
          add({ type: 'info', text: `🧠 Using project from conversation history: "${projectContext}"` });
        }
      }
      
      console.log('[handleClaudeSearch] Final Parsed Brands:', brands);
      console.log('[handleClaudeSearch] Final Project Context:', projectContext);
      // --- END OF FIX ---
