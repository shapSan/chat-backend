  ...
  try {
    switch (intent.tool) {
      case 'find_brands': {
        ...
        // Extract the actual project name from the message, not search keywords
        let actualProjectName = knownProjectName;
        
        if (!actualProjectName) {
          // Look for explicit project mentions in the message
          const projectPatterns = [
            /(?:for|about|called|titled|project:?)\s*["']?([^"'\n]+?)["']?(?:\s*(?:reboot|remake|sequel|series|movie|film|show|production))/i,
            /["']([^"']+?)["']\s*(?:reboot|remake|sequel|series|movie|film|show|production)/i,
            /(?:movie|film|show|series|production)\s*(?:called|titled|named)?\s*["']?([^"'\n]+?)["']?/i
          ];
          ...
          // If still no name, check if there's a clear title in structured fields
          if (!actualProjectName && /Synopsis:/i.test(userMessage)) {
            const titleMatch = userMessage.match(/(?:Title|Project|Film|Movie|Show):\s*([^\n]+)/i);
            if (titleMatch) actualProjectName = titleMatch[1].trim();
          }
          
          // Last resort: use a descriptive default
          if (!actualProjectName) {
            actualProjectName = genre ? `${genre.charAt(0).toUpperCase() + genre.slice(1)} Production` : 'Brand Partnership Project';
          }
        }
        ...
        // STEP 4: GENERATE CREATIVE CONTENT (This is the critical missing step)
        const top2Brands = finalBrands.slice(0, 2);
        add({ type: 'process', text: '✍️ Generating creative content for top 2 brands...' });
        
        // NOTE: This is where the main handler needs to call getTextResponseFromClaude
        // with the Airtable prompt to generate Integration, Why it works, and HB Insights
        // The actual AI call happens in the main handler, not here
        
        // STEP 5: PARSE AND POPULATE (structure for the main handler to fill)
        const parsedBrands = top2Brands.map(brand => ({
          name: brand.name,
          content: [
            `Integration: ${brand.category} partnership opportunity aligned with ${actualProjectName}`,
            `Why it works: ${brand.reason || 'Strong brand-production synergy'}`,
            `HB Insights: ${brand.insight?.rawText || `${brand.partnershipCount} partnerships, ${brand.dealsCount} active deals`}`
          ],
          // Include raw data for the main handler to use when calling AI
          _rawBrandData: brand,
          _needsAIGeneration: true  // Flag tells main handler to generate real content
        }));
        
        // STEP 6: FINALIZE THE RESPONSE PAYLOAD
        add({ type: 'complete', text: `✅ Response ready: ${top2Brands.length} detailed brands, ${brandSuggestionsForPicker.length} total options` });

        return {
          mcpThinking,
          organizedData: {
            // Main trigger flag for frontend
            dataType: 'BRAND_RECOMMENDATIONS',
            
            // For the cards: top 2 brands with creative content
            detailedBrands: parsedBrands,
            
            // For the picker: full list with minimal details
            brandSuggestions: brandSuggestionsForPicker,
            
            // The actual project name
            projectName: actualProjectName,
            
            // Context for the main handler to use when calling AI
            _aiGenerationContext: {
              projectName: actualProjectName,
              synopsis: search_term,
              genre: genre,
              top2BrandsRaw: top2Brands,
              distributorInfo: distributorForTerms,
              meetingsSnapshot: supportingContext.meetings.slice(0, 2),
              emailsSnapshot: supportingContext.emails.slice(0, 2)
            }
          },
        };
      }
