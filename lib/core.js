...
async function fetchCuratedBrandBuckets(partnershipData, { add }) {
  ...
  if (relevantCategories.length > 0) {
    ...
    categoryMatchedPromise = hubspotAPI.searchBrands({
      ...
      filterGroups: [{ filters: [
        { propertyName: 'client_status', operator: 'NEQ', value: 'Active' }, // Not already active
        { propertyName: 'main_category', operator: 'IN', values: relevantCategories }
      ]}]
    });
  } else {
    // Fallback to any non-active brands if no categories
    categoryMatchedPromise = hubspotAPI.searchBrands({
      limit: quotas.category,
      filterGroups: [{ filters: [
  ...
  // More flexible query: Inactive status OR modified 6-24 months ago
  ...
  const dormantPromise = hubspotAPI.searchBrands({
    ...
    filterGroups: [
      // Group 1: Inactive with past partnerships
      { filters: [
        { propertyName: 'client_status', operator: 'EQ', value: 'Inactive' },
        { propertyName: 'partnership_count', operator: 'GT', value: '0' }
      ]},
      // Group 2: Any brand modified 6-24 months ago (win-back opportunity)
      { filters: [
        { propertyName: 'hs_lastmodifieddate', operator: 'GTE', value: twentyFourMonthsAgo.toISOString() },
        { propertyName: 'hs_lastmodifieddate', operator: 'LTE', value: sixMonthsAgo.toISOString() },
        { propertyName: 'partnership_count', operator: 'GT', value: '0' }
      ]}
    ],
  ...
  const addBrandToMap = (brand, bucket) => {
    ...
    if (brandMap.has(brand.id)) {
      // Brand already exists, add the new tag
...
\`\`\``;
  ...
  allRankedBrands = allRankedBrands.map(brand => {
    ...
    // Base scores according to new specifications
    if (hasCommsDiscovery) {
      score += 150; // Comms Discovery base score
    }
    if (hasCategoryMatch) {
      score += 130; // Category Match base score
    }
    if (hasWinBack) {
      score += 120; // Win-Back base score
    }
    if (hasActiveClient) {
      score += 90; // Active Client base score
    }
    if (hasWildcard) {
      score += 70; // Wild Card base score
    }
