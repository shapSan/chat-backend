...
export function tagAndCombineBrands({
  ...
  const addBrandBase = (id, brand, primaryTag, reason, contextData) => {
    brandMap.set(id, {
      ...
      lastActivity: brand.properties.hs_lastmodifieddate,
      hubspotUrl: `https://app.hubspot.com/contacts/${hubspotAPI?.portalId || process.env.HUBSPOT_PORTAL_ID || '0'}/company/${brand.id}`,
      oneSheetLink: brand.properties.one_sheet_link || null,  // Include one-sheet
      secondaryOwnerId: brand.properties.secondary_owner || null,  // Store for later resolution
      specialtyLeadId: brand.properties.specialty_lead || null,    // Store for later resolution
      tags: [primaryTag],
      relevanceScore: 90,
      reason,
      insight: contextData,
      ...
      case 'find_brands': {
        ...
        if (!actualProjectName) {
          ...
          const projectPatterns = [
            ...
            // Pattern for "Full Project Name reboot/movie/etc" - STOP AT SYNOPSIS
            /([A-Z][^,\n.]+?)(?:\s+(?:reboot|remake|sequel|series|movie|film|show|production))?(?:\s*Synopsis:|\s*$)/i,
            // Look for capitalized multi-word phrases - STOP AT SYNOPSIS
            /^([A-Z][a-z]+(?:\s+[A-Z][a-z]+)+?)(?:\s*Synopsis:|\s*$)/
          ];
          ...
          for (const pattern of projectPatterns) {
            ...
            if (match && match[1]) {
              ...
              actualProjectName = actualProjectName.replace(/\s*Synopsis\s*$/i, '').trim();
              // Ensure we got the full name, not just first word
              if (actualProjectName.split(' ').length > 1 || actualProjectName.length > 10) {
                break;
              }
            }
          }
        ...
        const brandSuggestionsForPicker = finalBrands.map(brand => ({
          ...
          tags: brand.tags || [],
          hubspotUrl: brand.hubspotUrl || null,  // Include for isInSystem flag
...
Make the content creative, specific, and compelling. Focus on storytelling opportunities and marketing potential.`;
        ...
        if (creativeTextResponse && creativeTextResponse.length > 50) {
          ...
          parsedBrands = brandSections.slice(0, 2).map((section, idx) => {
            ...
            return {
              id: originalBrandData?.id, // <-- THE CRITICAL FIX: Add the ID here
              name: brandName,
              content: content,
          ...
          while (parsedBrands.length < 2 && parsedBrands.length < top2Brands.length) {
            ...
            const brand = top2Brands[idx];
            parsedBrands.push({
              id: brand?.id, // <-- Add the ID here too
              name: brand.name,
              content: [
          ...
          parsedBrands = top2Brands.slice(0, 2).map(brand => ({
            id: brand?.id, // <-- Add the ID in the fallback case
            name: brand.name,
            content: [
