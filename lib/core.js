...
async function fetchAllBrandData(intent, { add, knownProjectName, userMessage, partnershipData }) {
  ...
  if (aiGeneratedSubCategories.length > 0) {
    ...
    add({ type: 'warning', text: 'AI did not identify specific sub-categories. Falling back to general search.' });
  }

  searchPromises.activeBrands = hubspotAPI.searchBrands({
      limit: 10,
  ...
  searchPromises.dormantBrands = hubspotAPI.searchBrands({
      limit: 15,
      filterGroups: [{ filters: [{ propertyName: 'client_status', operator: 'EQ', value: 'Inactive' }] }],
      sorts: [{ propertyName: 'hs_lastmodifieddate', direction: 'DESCENDING' }]
  });

  const [settledResults, wildcardBrands] = await Promise.all([
      Promise.allSettled(Object.values(searchPromises)),
      generateWildcardBrands(partnershipData.synopsis || userMessage)
  ]);

  const [aiResult, activeResult, dormantResult] = settledResults;

  const aiContextBrands = aiResult?.status === 'fulfilled' ? aiResult.value : { results: [] };
  const activeBrands = activeResult?.status === 'fulfilled' ? activeResult.value : { results: [] };
  const dormantBrands = dormantResult?.status === 'fulfilled' ? dormantResult.value : { results: [] };

  add({ type: 'result', text: `🎯 Found ${aiContextBrands.results?.length || 0} AI matches` });
  add({ type: 'result', text: `💼 Found ${activeBrands.results?.length || 0} active & ${dormantBrands.results?.length || 0} dormant clients` });

  // --- Step 3: Return a Clean, Structured Object for the Ranking Function ---
  return {
      aiContextBrands,
      activeBrands,
      dormantBrands,
      wildcardBrands,
      communications: { meetings: [], emails: [] }, // Comms search can be re-integrated here if needed
  };
}
