...
async function fetchBrandsByName(brandNames, { add }) {
  ...
  // Search for each brand name with staggered delays to avoid rate limiting
  const searchPromises = brandNames.slice(0, 15).map((brandName, index) => 
    new Promise(resolve => 
      setTimeout(async () => {
        try {
          const result = await hubspotAPI.searchBrands({
            query: brandName,
            limit: 5,
      ...
      }, index * 250) // 250ms delay between each API call to avoid rate limiting
    )
  );
