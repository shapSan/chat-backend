...
export function tagAndCombineBrands({
  ...
  subCategoryBrands?.results?.forEach((b) => {
    ...
    if (!brandMap.has(brandId)) {
      ...
      br.relevanceScore = 50;  // Base score + context match bonus
      br.relevanceScore += 50;  // Context Match bonus
    } else {
      const br = brandMap.get(brandId);
      br.tags.push('🎯 Perfect Context Match');
      br.relevanceScore = Math.min(100, br.relevanceScore + 50);  // Context Match bonus
  ...
  activityBrands?.results?.forEach((b) => {
    ...
    if (!brandMap.has(brandId)) {
      ...
      const br = brandMap.get(brandId);
      br.relevanceScore = 50;  // Base score
      br.relevanceScore += 5;   // Recent Activity bonus
    } else {
      const br = brandMap.get(brandId);
      br.tags.push('📧 Recent Activity');
      br.relevanceScore = Math.min(100, br.relevanceScore + 5);  // Recent Activity bonus
      if (contextData && !br.insight) br.insight = contextData;
    }
  });
  ...
  synopsisBrands?.results?.forEach((b) => {
    ...
    if (!brandMap.has(brandId)) {
      ...
      const br = brandMap.get(brandId);
      br.relevanceScore = 50;  // Base score
      br.relevanceScore += 20; // Genre match bonus
    } else {
      const br = brandMap.get(brandId);
      br.tags.push('🎯 Genre Match');
      br.relevanceScore = Math.min(100, br.relevanceScore + 20);  // Genre match bonus
      if (contextData && !br.insight) br.insight = contextData;
    }
  });
  ...
  genreBrands?.results?.forEach((b) => {
    ...
    if (!brandMap.has(brandId)) {
      ...
      const br = brandMap.get(brandId);
      br.relevanceScore = 50;  // Base score
      br.relevanceScore += 15; // Vibe match bonus
    } else {
      const br = brandMap.get(brandId);
      br.tags.push('🎭 Vibe Match');
      br.relevanceScore = Math.min(100, br.relevanceScore + 15);  // Vibe match bonus
      if (contextData && !br.insight) br.insight = contextData;
    }
  });
  ...
  dormantBrands?.results?.forEach((b) => {
    ...
    if (!brandMap.has(brandId)) {
      ...
      const br = brandMap.get(brandId);
      br.relevanceScore = 50;  // Base score
      br.relevanceScore += 25; // Re-Ignition Opportunity bonus
    } else {
      const br = brandMap.get(brandId);
      br.tags.push('📈 Re-Ignition Opportunity');
      br.relevanceScore = Math.min(100, br.relevanceScore + 25);  // Re-Ignition Opportunity bonus
      if (contextData && !br.insight) br.insight = contextData;
    }
  });
  ...
  if (wildcardCategories?.length) {
    ...
    wildcardCategories.slice(0, 5).forEach((brandName, idx) => {
      ...
      brandMap.set(`wildcard_${idx}`, {
        ...
        relevanceScore: 60,  // Base score for wildcards
        reason: pitch,
        isWildcard: true,
      });
    });
...
async function fetchAllBrandData(intent, { add, knownProjectName, userMessage, partnershipData }) {
  ...
  if (openAIApiKey && productionContext) {
    try {
      const categoryGenerationPrompt = `Based on the following project context, act as a brand integration expert. Your task is to identify the 5 most relevant and natural-fit brand sub-categories to search for in our HubSpot database. The HubSpot field is 'product_sub_category__multi_'.

Project Context:
${productionContext}

Return ONLY a valid JSON array of strings. Example: ["Ride Sharing App", "High-End Watchmaker", "Cybersecurity Software"]`;
      ...
      if (aiResponse.ok) {
        ...
        if (responseText) {
          try {
            // Parse JSON response
            aiGeneratedSubCategories = JSON.parse(responseText);
            ...
            console.error('[fetchAllBrandData] Failed to parse AI sub-categories:', parseError);
          }
        }
      }
    } catch (error) {
  ...
  // Staggered name searches with delays to avoid rate limits
  for (const term of nameTerms.slice(0, 4)) {
    hubspotSearchPromises.push(
      (async () => {
        await new Promise(resolve => setTimeout(resolve, 250 * nameTerms.indexOf(term))); // Stagger requests
        return withTimeout(
  ...
  const nameSearchEndIdx = baseIndex + Math.min(nameTerms.length, 4);
  
  for (let i = baseIndex; i < nameSearchEndIdx; i++) {
  ...
  add({ type: 'info', text: `🔍 Found ${nameSearchResults.length} results from name searches` });
  ...
  return {
    subCategoryBrands: subCategoryResults,  // AI-driven sub-category results
...
function rankAndCombineBrands(dataPools, { search_term, add }) {
  ...
  const { subCategoryBrands, synopsisBrands, genreBrands, activeBrands, dormantBrands, wildcardBrands, communications } = dataPools;

  const finalBrands = tagAndCombineBrands({
    subCategoryBrands,  // Add the new AI-driven pool
    synopsisBrands,
    genreBrands,
    activeBrands,
    dormantBrands,  // Add the dormant pool
...
async function runBrandSearchOrchestrator(intent, { add, knownProjectName, userMessage, mcpThinking, conversationContext }) {
  // Extract project name and details from the user message if not already known
  let projectName = knownProjectName;
  let extractedData = {};
  
  // Stage 1: High-Confidence Regex (Fast Lane)
  if (!projectName) {
  ...
  // Stage 2: Primary AI Extraction (Bulletproof Method)
  // Always run this to get full production details
  if (openAIApiKey) {
    try {
      const extractionPrompt = `Analyze the following production details and extract the information into a structured JSON object. Identify the primary title, distributor, release date, cast, and a brief synopsis. Use null for any missing fields.
...
\`\`\``;
      ...
      if (extractResponse.ok) {
        ...
        if (extracted) {
          ...
          // Use AI-extracted title if we don't have a project name yet
          if (!projectName && extracted.title) {
            projectName = extracted.title;
            console.log('[runBrandSearchOrchestrator] AI extraction: found project title:', projectName);
          }
  ...
  const finalPartnershipData = {
    ...
    vibe: partnershipData?.vibe || partnershipData?.genre || null,
  ...
  // New logic to select a diverse trio for creative pitches
  const brandsForPitching = [];
  const seenIds = new Set();

  if (allRankedBrands.length > 0) {
    // 1. The top-ranked brand overall
    brandsForPitching.push(allRankedBrands[0]);
    seenIds.add(allRankedBrands[0].id);

    // 2. The best Vibe/Context match that isn't already included
    const vibeMatch = allRankedBrands.find(b => 
      !seenIds.has(b.id) && 
    ...
    if (vibeMatch) {
      brandsForPitching.push(vibeMatch);
      seenIds.add(vibeMatch.id);
    }

    // 3. The best Re-Ignition/Wildcard opportunity
    const opportunity = allRankedBrands.find(b => 
      !seenIds.has(b.id) && 
      (b.tags?.includes('📈 Re-Ignition Opportunity') || b.isWildcard)
    );
    if (opportunity && brandsForPitching.length < 3) {
      brandsForPitching.push(opportunity);
      seenIds.add(opportunity.id);
    }
    
    // Fallback: if we don't have enough diverse brands, fill from top-ranked
    while (brandsForPitching.length < 3 && brandsForPitching.length < allRankedBrands.length) {
      const nextBrand = allRankedBrands.find(b => !seenIds.has(b.id));
      if (nextBrand) {
        brandsForPitching.push(nextBrand);
        seenIds.add(nextBrand.id);
      } else {
        break;
      }
    }
  ...
  const enrichedBrands = await generateCreativeContent(brandsForPitching, {
    add,
