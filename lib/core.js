...
export async function handleClaudeSearch(
  ...
  try {
    switch (intent.tool) {
      case 'find_brands': {
        // Build synopsis- and vibe-aware searches
        const hasStructuredFields = /Synopsis:|Distributor:|Talent:|Location:|Release|Date:|Shoot/i.test(
          userMessage
        );
        ...
        const extractedTitle = knownProjectName || null;
        if (extractedTitle) add({ type: 'process', text: `📌 Using known project: "${extractedTitle}"` });
        ...
        // COMPLETE STRUCTURED DATA PAYLOAD FOR FRONTEND
        // Creating two separate arrays with different levels of detail
        
        // 1. FULL LIST with minimal details for the "More brands" picker UI
        ...
        // 2. TOP 2 BRANDS with full detailed information
        const top2Brands = finalBrands.slice(0, 2);
        const detailedBrandsWithContent = top2Brands.map(brand => ({
          // All original brand data
          ...brand,
          // Detailed content fields that will be populated by AI
          integration: `Strategic ${brand.category} integration opportunity`, // Placeholder - will be filled by AI
          whyItWorks: brand.reason || `Perfect match for production needs`, // Uses existing reason or placeholder
          hbInsights: brand.insight ? 
            (brand.insight.rawText || `${brand.partnershipCount} partnerships, ${brand.dealsCount} deals`) :
            `Active client with proven track record`,
          // Additional structured details
          details: {
            partnershipCount: brand.partnershipCount,
            dealsCount: brand.dealsCount,
            clientType: brand.clientType,
            lastActivity: brand.lastActivity,
            isWildcard: brand.isWildcard || false
          },
          // Flag for main handler to enhance with AI if needed
          needsAIEnhancement: true
        }));
        
        // 3. Extract or determine project name
        const projectName = knownProjectName || 
                          extractedTitle || 
                          (hasStructuredFields ? 'Production Project' : 'Brand Search') ||
                          'Brand Recommendations';
        
        add({ type: 'process', text: `📦 Structured payload ready: ${detailedBrandsWithContent.length} detailed brands, ${brandSuggestionsForPicker.length} total suggestions` });

        return {
          mcpThinking,
          organizedData: {
            // CRITICAL: Main trigger for frontend to render brand cards
            dataType: 'BRAND_RECOMMENDATIONS',
            
            // Full list with minimal details for picker UI (~17 brands)
            brandSuggestions: brandSuggestionsForPicker,
            
            // Top 2 brands with complete detailed information
            detailedBrands: detailedBrandsWithContent,
            ...
            // Additional metadata
            pools: {
              synopsisCount: synopsisBrands?.results?.length || 0,
              genreCount: genreBrands?.results?.length || 0,
              activeCount: activeBrands?.results?.length || 0,
              wildcardCount: wildcardBrands?.length || 0,
              totalProcessed: finalBrands.length
            },
            
            // Context for AI enhancement if needed
            context: {
              meetingsCount: supportingContext.meetings.length,
              emailsCount: supportingContext.emails.length,
              emailStatus: supportingContext.emailStatus,
              synopsis: search_term,
              genre: genre || 'general',
              hasStructuredFields: hasStructuredFields,
              searchTerms: {
                hubspot: hubspotTerms,
                communications: commsTerms
              }
            },
            ...
            metadata: {
              ...
              brandsForDetailedResponse: top2Brands.map(b => b.id) // Just IDs for reference
            }
          },
        };
      }
