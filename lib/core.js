...
async function fetchAllBrandData(intent, { add, knownProjectName, userMessage, partnershipData }) {
  ...
  // --- Step 1: Generate Smart Sub-Categories with the Powerful AI Prompt ---
  let aiGeneratedSubCategories = [];
  if (openAIApiKey && partnershipData.synopsis) {
    try {
      const productionContext = `
        Project Name: ${partnershipData.title || 'Unknown'}
        Genre: ${partnershipData.vibe || partnershipData.genre_production || 'N/A'}
        Cast: ${(partnershipData.cast || []).join(', ')}
        Synopsis: ${partnershipData.synopsis}
      `.trim();

      const categoryGenerationPrompt = `Your task is to act as an expert brand integration strategist. Analyze the provided project context. Your ONLY output must be a single, valid JSON array of strings containing the 5 most relevant and specific brand sub-categories to search for. CRITICAL RULES: 1. You MUST return ONLY a JSON array. 2. Do NOT include any conversational text or markdown formatting. Project Context: --- ${productionContext} --- JSON Array Output:`;
      ...
      const aiResponse = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          model: 'gpt-4o-mini',
          messages: [{ role: 'user', content: categoryGenerationPrompt }],
          temperature: 0.2,
          max_tokens: 150,
        }),
      });
      ...
      if (aiResponse.ok) {
        ...
        aiGeneratedSubCategories = Array.isArray(categoryData) ? categoryData.slice(0, 5) : [];
      }
    } catch (error) {
  ...
  // --- Step 2: Build and Execute HubSpot Search Promises ---
  // Use a NAMED object for promises, which is more robust than an array.
  const searchPromises = {
    activeBrands: hubspotAPI.searchBrands({
        limit: 10,
        filterGroups: [{ filters: [{ propertyName: 'client_status', operator: 'IN', values: ['Active', 'Contract'] }] }],
        sorts: [{ propertyName: 'partnership_count', direction: 'DESCENDING' }]
    }),
    dormantBrands: hubspotAPI.searchBrands({
        limit: 15,
        filterGroups: [{ filters: [{ propertyName: 'client_status', operator: 'EQ', value: 'Inactive' }] }],
  ...
  if (aiGeneratedSubCategories.length > 0) {
    ...
    // Add the AI promise to the object if it's needed
    searchPromises.aiContextBrands = hubspotAPI.searchBrands({
        limit: 20,
        filterGroups: [{ filters: [{ propertyName: 'product_sub_category__multi_', operator: 'IN', values: aiGeneratedSubCategories }] }],
        sorts: [{ propertyName: 'hs_lastmodifieddate', direction: 'DESCENDING' }]
    });
  } else {
    add({ type: 'warning', text: 'AI did not identify specific sub-categories.' });
  }

  // Communications search - Fireflies and O365
  const commsTerms = uniqShort([partnershipData.title, ...(Array.isArray(partnershipData.cast) ? partnershipData.cast : [])].filter(Boolean), 7);
  const commsPromise = Promise.allSettled([
      firefliesApiKey ? searchFireflies(commsTerms, { limit: 10 }) : Promise.resolve({ transcripts: [] }),
      o365API.searchEmails ? o365API.searchEmails(commsTerms, { limit: 10 }) : Promise.resolve({ emails: [] })
  ]).then(results => {
      const ffData = results[0]?.status === 'fulfilled' ? results[0].value : { transcripts: [] };
      const emData = results[1]?.status === 'fulfilled' ? results[1].value : { emails: [] };
      return { meetings: ffData.transcripts || [], emails: emData.emails || [] };
  });

  const [settledResults, wildcardBrands, communications] = await Promise.all([
      Promise.allSettled(Object.values(searchPromises)),
      generateWildcardBrands(partnershipData.synopsis || userMessage),
      commsPromise
  ]);

  // --- Step 3: Map results back from the named object ---
  const results = Object.keys(searchPromises).reduce((acc, key, index) => {
    const result = settledResults[index];
    acc[key] = result?.status === 'fulfilled' ? result.value : { results: [] };
    return acc;
  }, {});

  add({ type: 'result', text: `🎯 Found ${results.aiContextBrands?.results?.length || 0} AI matches` });
  add({ type: 'result', text: `💼 Found ${results.activeBrands?.results?.length || 0} active & ${results.dormantBrands?.results?.length || 0} dormant clients` });

  // Return a Clean, Structured Object for the Ranking Function
  return {
      aiContextBrands: results.aiContextBrands || { results: [] },
      activeBrands: results.activeBrands,
      dormantBrands: results.dormantBrands,
      wildcardBrands,
      communications,  // Real communications data from Fireflies and O365
  };
}
...
function rankAndCombineBrands(dataPools, { search_term, add }) {
  ...
  const { aiContextBrands, activeBrands, dormantBrands, wildcardBrands, communications } = dataPools;

  const finalBrands = tagAndCombineBrands({
    // Pass the aiContextBrands to the parameter that assigns the highest-value tag
    subCategoryBrands: aiContextBrands,
    synopsisBrands: { results: [] },  // Deprecated
    genreBrands: { results: [] },  // Deprecated
    activeBrands,
    dormantBrands,
