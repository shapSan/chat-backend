...
export async function handleClaudeSearch(
  ...
  try {
    switch (intent.tool) {
      case 'find_brands': {
        try { // Add a specific try/catch block to trap the hidden error
          console.log('[DEBUG] INTENT "find_brands" TRIGGERED. Starting process...');
          ...
          console.log('[DEBUG] CHECKPOINT 1: Project name identified.');
          ...
          const { commsTerms, hubspotTerms } = buildSearchTerms({
            title: actualProjectName,
            keywords: synopsisKeywords ? synopsisKeywords.split(' ') : [],
          });
          console.log('[DEBUG] CHECKPOINT 2: Keywords extracted and search terms built.');
          ...
          console.log(`[DEBUG] CHECKPOINT 3: Parallel searches complete. Counts: Synopsis(${synopsisBrands?.results?.length}), Active(${activeBrands?.results?.length}), Wildcards(${wildcardBrands?.length})`);
        ...
        if (firefliesApiKey || canSearchEmail) {
          ...
          console.log('[DEBUG] CHECKPOINT 4: Combining results and preparing for AI content generation...');
...
Make the content creative, specific, and compelling. Focus on storytelling opportunities and marketing potential.`;
        ...
        console.log('[DEBUG] CHECKPOINT 5: AI creative content generation complete.');
        console.log('[DEBUG] CHECKPOINT 6: Final data parsed and structured.');
        
        // Apply the definitive ID sanitization right before returning
        ...
        return {
          ...
          organizedData: {
            ...
            metadata: {
              ...
              genre: genre,
              distributor: partnershipMetadata?.distributor || null
            }
          },
        };
        
        } catch (caseError) {
            console.error('!!!!!!!!!!!!!!!!! FATAL ERROR WITHIN find_brands CASE !!!!!!!!!!!!!!!!!');
            console.error(caseError);
            add({ type: 'error', text: `Search failed: ${caseError.message}` });
            // Return null to allow graceful fallback, now with a detailed log of the actual error.
            return null;
        }
      }
