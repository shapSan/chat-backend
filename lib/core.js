...
/**
 * Generate intelligent, multi-layered search queries using AI
 */
async function generateSmartSearchQueries(synopsis, title, cast, { add }) {
  if (!openAIApiKey || !synopsis) {
    // Fallback to basic extraction
    return {
      primary_entities: [title].filter(Boolean).slice(0, 3),
      thematic_keywords: [],
  ...
  try {
    const prompt = `You are a search query expert for an entertainment brand partnership database. Based on the provided project synopsis, generate a JSON object containing a diverse and effective set of search terms for finding relevant meetings and emails.

The JSON object must have three keys:
1. "primary_entities": Extract up to 3 high-signal proper nouns. This MUST include the project title and key talent names.
2. "thematic_keywords": Extract up to 5 specific, thematic keywords from the plot. Avoid generic genres. Focus on tangible actions, objects, or concepts mentioned (e.g., "street racing", "parole", "heist", "animated thriller").
3. "strategic_categories": Suggest up to 4 relevant brand or business categories that might be discussed in a strategic meeting (e.g., "Streetwear fashion", "Luxury automotive", "Energy drinks").

Project Title: ${title || 'Unknown'}
Cast: ${Array.isArray(cast) ? cast.join(', ') : (cast || 'Unknown')}
...
Return ONLY the JSON object.`;
    ...
    if (result && result.primary_entities) {
      add({ type: 'info', text: `🎯 Generated search strategy: ${result.primary_entities.length} entities, ${result.thematic_keywords?.length || 0} themes, ${result.strategic_categories?.length || 0} categories` });
      return result;
    }
    
    throw new Error('Invalid AI response format');
  } catch (error) {
    console.error('[generateSmartSearchQueries] Error:', error.message);
    // Fallback to basic extraction
    return {
      primary_entities: [title, ...(Array.isArray(cast) ? cast.slice(0, 2) : [])].filter(Boolean),
      thematic_keywords: [],
      strategic_categories: []
    };
  }
...
export function tagAndCombineBrands({
  ...
  const sorted = Array.from(brandMap.values()).map(brand => {
    ...
    if (brand.clientStatus === 'Active' || brand.clientStatus === 'Contract') {
      ...
      // CRITICAL: Remove any re-ignition/win-back tags it might have
      brand.tags = brand.tags.filter(tag => 
        tag !== '📈 Re-Ignition Opportunity' && 
        tag !== '📈 Win-Back'
      );
    } else if (brand.clientStatus === 'Inactive') {
      // If it's inactive, it can be a re-ignition opportunity
      if (!brand.tags.includes('📈 Re-Ignition Opportunity') && !brand.tags.includes('📈 Win-Back')) {
    ...
    const isContextMatch = brand.tags.includes('🎯 Perfect Context Match') || 
                          brand.tags.includes('🎯 Genre Match') || 
                          brand.tags.includes('🎯 Category Match');
    const isWinBack = brand.tags.includes('📈 Re-Ignition Opportunity') || 
                     brand.tags.includes('📈 Win-Back');
    const isActiveClient = brand.tags.includes('🔥 Active Client');
    const isDiscovery = brand.tags.includes('🆕 Discovery');
    const isWildcard = brand.tags.includes('🚀 Wild Card') || brand.isWildcard;
...
async function runCommunicationsFirstSearch(intent, { add, knownProjectName, userMessage, mcpThinking, conversationContext }) {
  add({ type: 'start', text: '📡 Starting communications-first brand discovery...' });
...
\`\`\``;
  ...
  // Step 1: Search communications for clues based on the project theme
  const commsResults = await searchCommunicationsForClues(finalPartnershipData, { add });

  // Step 2: Use AI to extract brand names from those communications
  const brandNames = await extractBrandsFromComms(commsResults, { add });

  if (brandNames.length === 0) {
    add({ type: 'warning', text: 'No specific brands found in recent communications. Falling back to 4-bucket strategy.' });
    // Fall back to the existing curated buckets approach
    const traditionalResults = await fetchCuratedBrandBuckets(finalPartnershipData, { add });
    
    // Generate creative content for top brands
    const topBrands = traditionalResults.brands.slice(0, 2);
    const creativeResult = await generateCreativeContent(topBrands, {
      add,
      actualProjectName: projectName,
      search_term: userMessage,
      partnershipData: finalPartnershipData,
      communications: traditionalResults.communications
    });
    
    return prepareFrontendPayload({
      enrichedBrands: creativeResult.enrichedBrands,
      allRankedBrands: traditionalResults.brands,
  ...
  // Step 3: Fetch the full details for the extracted brands from HubSpot
  const hubspotBrands = await fetchBrandsByName(brandNames, { add });

  // Combine with traditional buckets for diversity (smaller quotas)
  add({ type: 'info', text: '🔄 Enhancing with traditional discovery methods...' });
  const traditionalResults = await fetchCuratedBrandBuckets(finalPartnershipData, { add });
  
  // Merge the results, prioritizing comms-discovered brands
  const brandMap = new Map();
  
  // Add comms-discovered brands first (highest priority)
  hubspotBrands.forEach(brand => {
    if (brand.id) {
      brand.tags = [...(brand.tags || []), '💬 Comms Discovery'];
      brand.sourceBucket = '💬 Comms Discovery';
      brand.relevanceScore = 150; // High base score for being found in comms
      brand.reason = brand.reason || `Discovered through recent ${brand.name} communications and discussions`;
      brandMap.set(brand.id, brand);
    }
  });
  
  // Add traditional bucket brands (if not already present)
  traditionalResults.brands.forEach(brand => {
    if (brand.id && !brandMap.has(brand.id)) {
      // Slightly lower the score for non-comms brands to prioritize comms-discovered ones
      brand.relevanceScore = (brand.relevanceScore || 100) * 0.8;
      brandMap.set(brand.id, brand);
    }
  });
  
  // Sort by relevance score
  let allRankedBrands = Array.from(brandMap.values())
    .map(brand => {
      // Apply new weighted scoring system
      let score = 0;
      
      const hasCommsDiscovery = brand.tags?.includes('💬 Comms Discovery');
      const hasCategoryMatch = brand.tags?.some(tag => tag.includes('🎯 Category Match'));
      const hasWinBack = brand.tags?.some(tag => tag.includes('📈 Win-Back'));
      const hasActiveClient = brand.tags?.includes('🔥 Active Client');
      const hasWildcard = brand.tags?.includes('🚀 Wild Card');
      
      // Base scores
      if (hasCommsDiscovery) score += 150;
      if (hasCategoryMatch) score += 130;
      if (hasWinBack) score += 120;
      if (hasActiveClient) score += 90;
      if (hasWildcard) score += 70;
      
      // Discovery bonus
      if (hasCategoryMatch && !hasActiveClient) score += 50;
      
      // Partnership count bonus
...
/**
 * Search communications broadly for project-related conversations
 */
async function searchCommunicationsForClues(partnershipData, { add }) {
  // Generate smart search queries using the new AI-powered function
  const searchQueries = await generateSmartSearchQueries(
    partnershipData.synopsis,
    partnershipData.title,
    partnershipData.cast,
    { add }
  );
  
  // Combine all search terms, prioritizing by layer
  const allSearchTerms = [
    ...searchQueries.primary_entities,
    ...searchQueries.thematic_keywords,
    ...searchQueries.strategic_categories
  ].filter(Boolean);
  
  if (allSearchTerms.length === 0) {
    add({ type: 'warning', text: '⚠️ No search terms generated, using title as fallback' });
    allSearchTerms.push(partnershipData.title || 'production');
  }
  
  // Execute layered search strategy
  const allMeetings = new Map();
  const allEmails = new Map();
  
  // Layer 1: Primary entities (highest signal)
  if (searchQueries.primary_entities.length > 0) {
    add({ type: 'info', text: `🔍 Layer 1: Searching for primary entities: ${searchQueries.primary_entities.join(', ')}` });
    const [ffResult, o365Result] = await Promise.allSettled([
      firefliesApiKey ? searchFireflies(searchQueries.primary_entities, { limit: 10 }) : Promise.resolve({ transcripts: [] }),
      o365API.searchEmails ? o365API.searchEmails(searchQueries.primary_entities, { limit: 10 }) : Promise.resolve({ emails: [] })
    ]);
  ...
  // Layer 2: Thematic keywords (contextual matches)
  if (searchQueries.thematic_keywords.length > 0) {
    add({ type: 'info', text: `🔍 Layer 2: Searching for themes: ${searchQueries.thematic_keywords.slice(0, 3).join(', ')}...` });
    const [ffResult, o365Result] = await Promise.allSettled([
      firefliesApiKey ? searchFireflies(searchQueries.thematic_keywords, { limit: 8 }) : Promise.resolve({ transcripts: [] }),
      o365API.searchEmails ? o365API.searchEmails(searchQueries.thematic_keywords, { limit: 8 }) : Promise.resolve({ emails: [] })
    ]);
    
    if (ffResult.status === 'fulfilled' && ffResult.value.transcripts) {
      ffResult.value.transcripts.forEach(t => {
        if (!allMeetings.has(t.id)) allMeetings.set(t.id, t);
      });
    }
    if (o365Result.status === 'fulfilled' && o365Result.value.emails) {
      o365Result.value.emails.forEach(e => {
        if (!allEmails.has(e.id)) allEmails.set(e.id, e);
      });
    }
  }
  
  // Layer 3: Strategic categories (discovery opportunities)
  if (searchQueries.strategic_categories.length > 0) {
    add({ type: 'info', text: `🔍 Layer 3: Searching for categories: ${searchQueries.strategic_categories.slice(0, 3).join(', ')}...` });
    const [ffResult, o365Result] = await Promise.allSettled([
      firefliesApiKey ? searchFireflies(searchQueries.strategic_categories, { limit: 5 }) : Promise.resolve({ transcripts: [] }),
      o365API.searchEmails ? o365API.searchEmails(searchQueries.strategic_categories, { limit: 5 }) : Promise.resolve({ emails: [] })
    ]);
  ...
  add({ type: 'result', text: `📊 Found ${meetings.length} relevant meetings & ${emails.length} emails across ${Object.keys(searchQueries).filter(k => searchQueries[k].length > 0).length} search layers` });
...
async function fetchCuratedBrandBuckets(partnershipData, { add }) {
  ...
  const quotas = { active: 5, category: 5, dormant: 4, discovery: 3, wildcard: 5 };
  ...
  const addBrandToMap = (brand, bucket, isPrimary = true) => {
    ...
    if (brandMap.has(brand.id)) {
      // Brand already exists, only add as secondary tag
      const existingBrand = brandMap.get(brand.id);
      if (!existingBrand.tags.includes(bucket)) {
        existingBrand.tags.push(bucket);
      }
      // Update reason if it's more specific
      if (!existingBrand.reason || existingBrand.reason.length < 20) {
        existingBrand.reason = generateReason(brand.properties || brand, partnershipData);
      }
      return;
    }
  ...
  // 4. 🆕 Discovery (new prospects)
  add({ type: 'result', text: `🆕 Discovery Prospects: ${discoveryResults.results.length}` });
  discoveryResults.results.forEach(b => addBrandToMap(b, '🆕 Discovery'));
  ...
  finalBrandList = finalBrandList.map(brand => {
    // Add secondary tags based on criteria
    const tags = [...brand.tags]; // Start with existing tags
    
    // Check if brand qualifies for additional tags (but wasn't primary)
    if (!tags.includes('🎯 Category Match') && relevantCategories.includes(brand.category)) {
      tags.push('🎯 Category Match');
    }
    
    // Check for activity-based tags
    const partnershipCount = parseInt(brand.partnershipCount || '0');
    const dealsCount = parseInt(brand.dealsCount || '0');
    
    if (partnershipCount >= 10 && !tags.some(t => t.includes('Partnership'))) {
      tags.push(`🤝 ${partnershipCount} Partnerships`);
    }
    
    if (dealsCount >= 5 && !tags.some(t => t.includes('Deal'))) {
