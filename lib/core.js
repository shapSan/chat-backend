...
async function runQuickMatchOrchestrator(
  ...
  // STEP 3: Use the smartest AI for a single, high-quality match
  add({ type: 'process', text: '🤖 Using GPT-4o for high-quality matching...' });
  ...
  From this list of available brands, select the 10 with the most natural and creative fit.
  BRANDS AVAILABLE:
  ${brandList}

  Return ONLY a valid JSON object with your top 10 selections in the format:
  {
    "selections": [
  ...
  try {
    const response = await withTimeout(
      fetch('https://api.openai.com/v1/chat/completions', {
        ...
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${openAIApiKey}` },
        body: JSON.stringify({
          model: MODELS.openai.chat, // Using the best model as requested
    ...
    add({ type: 'result', text: `✅ AI identified ${selections.length} top matches.` });
    
    // STEP 4: Map results back to full brand objects to ensure all fields are present
    ...
    // STEP 5: Prepare the final data structure for the frontend
    return prepareFrontendPayload({
      enrichedBrands: [], // No deep-dive "enriched" brands
      allRankedBrands: matchedBrands, // This list will populate the brand picker
      mcpThinking,
      projectName: partnershipData.title || projectName,
      partnershipData,
      communications: { meetings: [], emails: [] }, // Empty comms
      finalReplyText: `Here are ${matchedBrands.length} quick match suggestions for ${partnershipData.title || projectName}.`
    });
