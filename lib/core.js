...
async function fetchAllBrandData(intent, { add, knownProjectName, userMessage, partnershipData }) {
  ...
  if (openAIApiKey && (partnershipData?.vibe || partnershipData?.synopsis || search_term)) {
    try {
      ...
      const categoryResponse = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'user',
              content: `Based on this production context, provide the top 3 most relevant business search terms for finding meeting transcripts (e.g., 'Live Entertainment', 'Tour Sponsorship', 'Artist Collaboration'). Respond with only a comma-separated list.\n\nContext: ${contextForCategories.slice(0, 300)}`
            }
          ],
          temperature: 0.3,
          max_tokens: 50
      ...
      if (categoryResponse.ok) {
        ...
        const categoriesText = categoryData.choices?.[0]?.message?.content?.trim();
        if (categoriesText) {
          businessCategories = categoriesText.split(',').map(cat => cat.trim()).slice(0, 3);
  ...
  // Define known HubSpot categories for precise filtering
  const KNOWN_HUBSPOT_CATEGORIES = [
    'Food & Beverage', 'Fashion & Apparel', 'Electronics & Appliances',
    'Health & Beauty', 'Home & Garden', 'Automotive', 'Entertainment',
    'Sports & Fitness', 'Gaming', 'Travel', 'Baby Care', 'Floral',
    'Security', 'Education', 'Financial Services', 'Technology'
  ];
  ...
  hubspotTerms.forEach(term => {
    // Check if term matches a known category (case-insensitive)
    const termLower = term.toLowerCase();
    const matchedCategory = KNOWN_HUBSPOT_CATEGORIES.find(cat => 
      cat.toLowerCase().includes(termLower) || termLower.includes(cat.toLowerCase())
    );
    
    if (matchedCategory) {
      categoryTerms.push(matchedCategory);
    } else {
      nameTerms.push(term);
    }
  });
  ...
  const uniqueCategories = [...new Set(categoryTerms)];
  
  add({ type: 'info', text: `📂 Categories: ${uniqueCategories.join(', ') || 'none'}, 🏷️ Names: ${nameTerms.slice(0, 3).join(', ') || 'none'}` });
  
  // New communications terms: project name + business categories
  const commsTerms = [];
  if (knownProjectName) {
    commsTerms.push(knownProjectName);
  }
  commsTerms.push(...businessCategories);
  
  // Fallback: if no business categories were generated, use a subset of keywords
  ...
  const [hubspotResults, commsResults, wildcardResults] = await Promise.all([
    // HubSpot searches - now using hybrid strategy
    Promise.allSettled([
      // Category search - efficient IN operator for all categories at once
      ...
      // Parallel name searches - up to 4 terms
      ...nameTerms.slice(0, 4).map(term =>
        withTimeout(
          searchBrandsWithRetry({ query: term, limit: 3, searchType: 'keywords' }),
          4000,
          { results: [] }
        )
      ),
      
      // Keep the active clients search
      withTimeout(
        searchBrandsWithRetry({
          limit: 10,
          filterGroups: [
  ...
  // Consolidate results from hybrid search
  const categoryResults = hubspotResults[0].status === 'fulfilled' ? hubspotResults[0].value : { results: [] };
  
  // Combine name search results (indices 1-4 are name searches)
  const nameSearchResults = [];
  for (let i = 1; i <= nameTerms.slice(0, 4).length; i++) {
    if (hubspotResults[i]?.status === 'fulfilled' && hubspotResults[i].value?.results) {
      nameSearchResults.push(...hubspotResults[i].value.results);
    }
  }
  
  // The last result is always the active clients search
  const activeIdx = hubspotResults.length - 1;
  const activeResults = hubspotResults[activeIdx]?.status === 'fulfilled' ? hubspotResults[activeIdx].value : { results: [] };
  
  const categoryCount = categoryResults.results?.length || 0;
  const nameCount = nameSearchResults.length;
  const activeCount = activeResults.results?.length || 0;
  ...
  add({ type: 'result', text: `📊 HubSpot Found: ${categoryCount} category matches, ${nameCount} name matches, ${activeCount} active clients.` });
...
async function generateCreativeContent(topBrands, { add, actualProjectName, search_term, partnershipData }) {
  ...
  // Use the Airtable AI prompt structure
...
async function runBrandSearchOrchestrator(intent, { add, knownProjectName, userMessage, mcpThinking, conversationContext }) {
  // Stage 1: Check for knownProjectName (fastest)
  let projectName = knownProjectName;
  
  // Stage 2: Multi-Regex Waterfall - if no project name provided
  if (!projectName) {
    const regexPatterns = [
      // 1. NEW: Explicit command: "find brands for 'Project X'"
      /(?:find|get|search|recommend|suggest)\s+(?:me\s+)?(?:brands|matches|partners)\s+for\s+["']?([^"'\n]+?)["']?$/i,
      // 2. Explicit labels: "title: 'The Movie'", "project: 'Project X'"
      /(?:project|title|film|show|series|movie)(?:\s+(?:is|called|titled|name))?\s*[:\-]?\s*["']([^"']+)["']/i,
      // 3. Title at the start, before "Production:" or "Synopsis:" (would have fixed 'Kygo' issue)
      /^(.*?)(?: Production:| Synopsis:)/is,
      // 4. Action phrases: "working on 'The Movie'"
      /(?:working on|producing|developing)\s+["']([^"']+)["']/i,
      // 5. Contextual clues: "'The Movie' starring..."
      /["']([^"']+)["']\s+(?:starring|featuring|with a cast of)/i
    ];
    
    // Try each pattern in order, stop at first match
    for (let i = 0; i < regexPatterns.length; i++) {
      const match = userMessage.match(regexPatterns[i]);
      if (match && match[1]) {
        projectName = match[1].trim();
        console.log(`[runBrandSearchOrchestrator] Extracted project name using pattern ${i + 1}: "${projectName}"`);
        break;
      }
    }
  }
  
  // Stage 3: AI Fallback - only if both previous stages failed
  if (!projectName && openAIApiKey) {
    try {
      console.log('[runBrandSearchOrchestrator] No regex match found, using AI fallback');
      const aiResponse = await fetch('https://api.openai.com/v1/chat/completions', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${openAIApiKey}` },
        body: JSON.stringify({
          ...
          messages: [
            { 
              role: 'user', 
              content: `Review the following text and extract the primary movie, series, or production title. Respond with ONLY the title and nothing else.\n\n${userMessage.slice(0, 500)}` 
            }
          ],
          temperature: 0,
          max_tokens: 50
      ...
      if (aiResponse.ok) {
        ...
        if (extractedTitle && extractedTitle.length > 2 && extractedTitle.length < 100) {
          projectName = extractedTitle;
          console.log(`[runBrandSearchOrchestrator] AI extracted project name: "${projectName}"`);
        }
      }
    } catch (error) {
      console.error('[runBrandSearchOrchestrator] AI extraction failed:', error.message);
    }
  }
  ...
  // Extract production data from user message using AI - ALWAYS do this to get Cast, Location, Vibe
  let extractedData = {};
  if (openAIApiKey) {
    try {
      const extractionPrompt = `Extract production/partnership information from this text. Look for any mentions of:
- Studio or distributor names
- Release dates or production dates
- Cast members or talent names  
- Locations or settings
- Genre, vibe, or tone descriptions
- Production type (film, series, etc)
...
Return ONLY valid JSON with these fields (use null for missing data):
{
  ...
  "productionType": "type (film/series/etc)",
  "location": "production location",
  "cast": "main cast members",
  "vibe": "genre/tone/vibe"
}
...
Text: ${userMessage}`;
      ...
      const extractResponse = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          model: 'gpt-4o-mini',
          messages: [
            { role: 'system', content: 'You are a data extraction assistant. Extract only what is explicitly mentioned. Return valid JSON.' },
      ...
      if (extractResponse.ok) {
        ...
        if (extracted) {
          ...
          add({ type: 'info', text: '📋 Extracted production details from message' });
        }
      }
    } catch (error) {
  ...
  const finalPartnershipData = {
    ...
    productionStartDate: partnershipData?.startDate || partnershipData?.production_start_date || extractedData.productionStartDate || null,
    productionType: partnershipData?.productionType || partnershipData?.production_type || extractedData.productionType || null,
    location: partnershipData?.location || extractedData.location || null,
    cast: partnershipData?.cast || extractedData.cast || null,
    vibe: partnershipData?.vibe || partnershipData?.genre || extractedData.vibe || null,
    budget: partnershipData?.budget || null,
    synopsis: partnershipData?.synopsis || null
  };
  ...
  if (finalPartnershipData.cast) productionDataInfo.push(`Cast: ${finalPartnershipData.cast}`);
