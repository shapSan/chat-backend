  ...
  try {
    switch (intent.tool) {
      case 'find_brands': {
        ...
        // STEP 4: STRUCTURE FOR CREATIVE AI CALL (main handler will execute)
        add({ type: 'process', text: '✍️ Preparing for creative content generation...' });
        
        // STEP 5: PARSE AND POPULATE THE FINAL DATA
        // Create the structure that main handler will populate with AI-generated content
        const parsedBrands = top2Brands.map(brand => ({
          name: brand.name,
          content: [
            `Integration: ${brand.category} partnership opportunity for ${actualProjectName}`,
            `Why it works: ${brand.reason || 'Strong brand-production alignment'}`,
            `HB Insights: ${brand.insight?.rawText || `${brand.partnershipCount} partnerships, ${brand.dealsCount} active deals`}`
          ],
          // Critical: Include all raw data needed for AI generation
          _rawBrandData: {
            ...brand,
          ...
          _needsAIGeneration: true  // Flag for main handler to call getTextResponseFromClaude
        }));
        
        add({ type: 'complete', text: `✅ Structure ready for AI enhancement: ${top2Brands.length} brands selected from ${finalBrands.length} total` });
        ...
        return {
          ...
          organizedData: {
            ...
            // Top 2 brands with structure for creative content
            ...
            // Context package for main handler to use with Airtable prompt
            _aiGenerationPackage: {
              projectName: actualProjectName,
              synopsis: search_term,
              genre: genre || 'general',
              distributor: distributorForTerms || 'Independent',
              top2Brands: top2Brands.map(b => ({
                name: b.name,
                category: b.category,
                ...
                reason: b.reason,
                tags: b.tags,
                insight: b.insight?.rawText
              })),
              communicationsContext: {
                recentMeetings: supportingContext.meetings.slice(0, 2).map(m => ({
                  title: m.title,
                  date: m.dateString,
                  summary: m.summary?.overview
                })),
                recentEmails: supportingContext.emails.slice(0, 2).map(e => ({
                  subject: e.subject,
                  from: e.fromName,
                  date: e.receivedDate
                }))
