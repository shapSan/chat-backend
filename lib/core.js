...
async function fetchAllBrandData(intent, { add, knownProjectName, userMessage }) {
  ...
  add({ type: 'result', text: `📊 Data acquisition complete.` });
...
async function generateCreativeContent(topBrands, { add, actualProjectName, search_term }) {
  ...
  try {
    const aiResponse = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'gpt-4o-mini',
        messages: [
          { role: 'system', content: 'You are a creative brand partnership specialist.' },
  ...
  if (!creativeTextResponse) {
    add({ type: 'info', text: '⚠️ AI generation failed, using defaults.' });
    // Fallback: Return original brands with default content
    return topBrands.map((brand) => ({
      ...brand,
      content: [
        `Integration: Strategic product placement for ${brand.name}.`,
        `Why it works: Perfect alignment with project themes.`,
        `HB Insights: ${brand.partnershipCount} previous partnerships show strong potential.`,
      ],
    }));
  }
  ...
  return topBrands.map((originalBrandData, idx) => {
    const section =
      brandSections.find((s) =>
        s.toLowerCase().startsWith(` ${originalBrandData.name.toLowerCase()}`)
      ) || brandSections[idx];
    if (!section) return { ...originalBrandData, content: ['Default content.'] };
    ...
    return {
      ...originalBrandData, // This carries over the original ID and all other data
      content: contentLines.length > 0 ? contentLines : [section.trim()],
    };
  });
}
