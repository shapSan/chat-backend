...
async function fetchCuratedBrandBuckets(partnershipData, { add }) {
  ...
  const addBrandToMap = (brand, bucket, reason) => {
  ...
  activeResults.results.forEach(b => addBrandToMap(b, '🔥 Active Client', 'Currently engaged - easiest to activate'));

  add({ type: 'result', text: `🎯 Category Matches: ${categoryResults.results.length}` });
  categoryResults.results.forEach(b => addBrandToMap(b, '🎯 Category Match', 'Fresh prospect that fits the content perfectly'));

  add({ type: 'result', text: `📈 Dormant / Win-Back: ${dormantResults.results.length}` });
  dormantResults.results.forEach(b => addBrandToMap(b, '📈 Win-Back', 'Previous relationship ready to rekindle'));
...
\`\`\``;
  ...
  const allRankedBrands = await fetchCuratedBrandBuckets(finalPartnershipData, { add });
