...
EVIDENCE_PROVIDED: ${brand.evidence || "NONE"}
</BRAND_${idx + 1}>
`).join('\n')}
...
3. **HB Insights:** Follow these rules EXACTLY:
   - If EVIDENCE_PROVIDED contains a meeting/email reference, copy it EXACTLY.
   - If EVIDENCE_PROVIDED is "NONE", write a specific insight about why this brand-production pairing makes sense. Examples:
     * "Comedy-horror crossover appeals to Gen Z audience that drives [brand category] sales."
     * "Atlanta production hub aligns with brand's Southeast market expansion."
     * "Millennial nostalgia factor connects with brand's core 25-40 demographic."
     * "Meta-horror themes create natural integration opportunities for [specific product use]."
   - NEVER mention partnership counts, database status, or "fresh creative idea" generic phrases.
