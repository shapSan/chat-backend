...
async function getSemanticallyRankedBrands(partnershipData, { add }) {
  // Check cache first
  add({ type: 'process', text: '📦 Checking brand cache...' });
  
  const cached = await kv.get('hubspot-brand-cache');
  if (!Array.isArray(cached) || cached.length === 0) {
    add({ type: 'warning', text: '⚠️ Brand cache empty or invalid.' });
    return [];
  }
  
  add({ type: 'info', text: `📦 Cache hit: ${cached.length} brands available` });
  
  if (!openAIApiKey) {
    add({ type: 'warning', text: '⚠️ AI ranking unavailable (no API key). Using cache as-is.' });
    return cached.slice(0, 30);
  }
  
  try {
    add({ type: 'process', text: `🤖 Ranking ${cached.length} brands via AI...` });
    
    // Prepare brands for AI ranking
    const brandsForAI = cached.slice(0, 100).map(b => ({
      id: b.id,
      name: b.properties?.brand_name || b.brand_name || 'Unknown',
      category: b.properties?.main_category || b.main_category || 'General',
      subcategories: b.properties?.product_sub_category__multi_ || b.product_sub_category__multi_ || '',
      partnershipCount: parseInt(b.properties?.partnership_count || b.partnership_count || 0),
      clientStatus: b.properties?.client_status || b.client_status || '',
    }));
    
    const rankingPrompt = `You are a brand matching expert. Rank these brands by relevance to the production.
...
Return a JSON array of the top 30 brands with relevance scores (0-100):
[{"id": "...", "score": 95, "reason": "..."}, ...]`;
    ...
    const response = await withTimeout(
      fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          model: MODELS.openai.chatMini,
          messages: [
            { role: 'system', content: 'You are a brand relevance expert. Return only valid JSON arrays.' },
    ...
    if (!response || !response.ok) {
      add({ type: 'warning', text: '⚠️ AI ranking failed. Using fallback.' });
      return [];
    }
    ...
    if (!Array.isArray(ranked) || ranked.length === 0) {
      add({ type: 'warning', text: '⚠️ AI returned invalid format. Using fallback.' });
      return [];
    }
    
    // Map back to full brand objects
    const rankedBrands = ranked.slice(0, 30).map(r => {
      const original = cached.find(b => b.id === r.id);
      if (!original) return null;
      
      return {
        ...original,
        id: original.id,
        name: original.properties?.brand_name || original.brand_name || 'Unknown',
        brand: original.properties?.brand_name || original.brand_name || 'Unknown',
        category: original.properties?.main_category || original.main_category || 'General',
        subcategories: original.properties?.product_sub_category__multi_ || '',
        clientStatus: original.properties?.client_status || '',
        clientType: original.properties?.client_type || '',
        partnershipCount: original.properties?.partnership_count || '0',
        dealsCount: original.properties?.deals_count || '0',
        relevanceScore: r.score || 50,
        reason: r.reason || generateReason(original.properties || original, partnershipData),
        tags: ['🤖 AI Ranked'],
        source: 'ai-ranking'
      };
    }).filter(Boolean);
    
    add({ type: 'result', text: `✅ AI ranked ${rankedBrands.length} brands. Top: ${rankedBrands.slice(0, 5).map(b => b.name).join(' • ')}` });
    
    return rankedBrands;
  } catch (error) {
    console.error('[getSemanticallyRankedBrands] Error:', error.message);
    add({ type: 'warning', text: `⚠️ AI ranking error: ${error.message}` });
    return [];
  }
}
