...
async function getSemanticallyRankedBrands(partnershipData, { add }) {
  ...
  // REFINED PROMPT: This prompt is now more detailed to ensure strategic, non-repetitive results.
  const rankingPrompt = `You are a world-class brand strategist for Hollywood. Your task is to find the best brand partnerships for a new production.
Analyze the production context and the provided list of brands. Return a ranked list of the TOP 40 brands that are the best semantic and strategic fit.
...
<SCORING_CRITERIA>
1.  **Creative & Thematic Fit (Highest Weight):** How deeply does the brand's identity, category, and subcategories connect with the production's synopsis, genre, and vibe? A horror film should get brands that fit its specific themes (e.g., security systems, flashlights, or even something ironically cheerful), not just generic brands. Find the "absolutely great natural connections."
2.  **Audience Alignment:** Does the brand's target demographic align with the viewers of this specific content?
3.  **Strategic Value & Uniqueness (CRITICAL):** Do not just return the same obvious brands for every project. Prioritize unique, non-obvious, or "wildcard" suggestions that create an "aha!" moment. A brand with fewer partnerships but a perfect creative fit is more valuable than a frequent partner with a mediocre fit. AVOID REPETITIVE SUGGESTIONS.
4.  **Partnership History:** Consider 'partnershipCount' as a sign of experience, but it is the least important factor. A high count does not guarantee a good fit for THIS project.

<OUTPUT_INSTRUCTIONS>
- Return ONLY a valid JSON array.
- The array should contain objects for the top 40 brands, ranked by a holistic evaluation of the criteria above.
- Each object must have these keys: {"id": "<brand_id>", "score": <0-100>, "reason": "<A brief, insightful reason for the UNIQUE match to THIS project>"}.
`;
  ...
  try {
    const response = await withTimeout(
      fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          temperature: 0.6, // Increased slightly to encourage more creative and varied results
          max_tokens: 4000
        })
      }),
      20000,
    ...
    const preFilteredBrands = rankedResults.map(r => {
      ...
      return { ...originalBrand, relevanceScore: r.score || 50, reason: r.reason, tags: ['🤖 AI Ranked'], source: 'ai-ranking' };
    }).filter(Boolean);

    add({ type: 'result', text: `✅ AI pre-filtered to ${preFilteredBrands.length} top candidates.` });
...
\`\`\``;
  ...
  if (extractKeywordsForContextSearch) {
    try {
      const keywords = await extractKeywordsForContextSearch(finalPartnershipData);
      if (Array.isArray(keywords) && keywords.length) {
        add({ type: 'info', text: `🔑 Keywords: ${keywords.slice(0, 12).join(', ')}` });
      }
    } catch (e) {
  ...
  // --- REFINED FINAL SELECTION LOGIC ---
  // This logic explicitly selects two brands for the creative write-up:
  // 1. The Best "Safe Bet": An in-system brand with a strong creative fit.
  // 2. The Best "Creative Bet": A new or non-client brand that is an exciting wildcard.
  add({ type: 'process', text: '🧠 Selecting the top 2 strategic pitches...' });

  const brandsForPitching = [];
  const usedIds = new Set();
  
  // Define what "in-system" (a safe bet) means for this selection.
  const isInSystem = (brand) => 
      brand.clientStatus === 'Active' || 
      brand.clientStatus === 'Contract' || 
      (brand.clientStatus === 'Inactive' && (brand.partnershipCount || 0) > 5);

  // 1. Find the best SAFE BET (In-System)
  // This is the highest-scoring brand from the AI's list that is also a trusted, existing partner.
  const safeBet = allRankedBrands.find(b => isInSystem(b));
  if (safeBet) {
    brandsForPitching.push(safeBet);
    usedIds.add(safeBet.id);
    add({ type: 'info', text: `👍 Safe Bet: ${safeBet.name} (proven partner)` });
  }

  // 2. Find the best CREATIVE BET (Not In-System)
  // This is the highest-scoring brand that is NOT a safe bet, prioritizing a fresh idea.
  const creativeBet = allRankedBrands.find(b => !isInSystem(b) && !usedIds.has(b.id));
  if (creativeBet) {
    brandsForPitching.push(creativeBet);
    usedIds.add(creativeBet.id);
    add({ type: 'info', text: `🚀 Creative Bet: ${creativeBet.name} (fresh idea)` });
  }

  // 3. Fallback: If either slot is empty, fill with the next best overall brand.
