      ...
      function: {
        ...
        parameters: {
          ...
          properties: { 
            project_name: { 
              type: 'string',
              description: 'The name of the project/production (e.g., "17 Sundays", "Fast 11")' 
            },
            synopsis: { 
              type: 'string',
              description: 'Plot description or story overview if provided' 
            },
            cast: { 
              type: 'array',
              items: { type: 'string' },
              description: 'Main cast members if mentioned'
            },
            genre: { 
              type: 'string',
              description: 'Genre or vibe (e.g., "Action", "Drama", "Thriller")' 
            },
            requested_category: { 
              type: 'string',
              description: 'Specific brand category if requested (e.g., "Technology", "Food & Beverage", "Automotive")'
            }
          },
      ...
      function: {
        ...
        parameters: {
          ...
          properties: { 
            ...
            aka_name: { 
              type: 'string',
              description: 'Alternate or working title if mentioned (e.g., "The Land" for "17 Sundays aka The Land")'
            },
            synopsis: { 
              type: 'string',
              description: 'Plot description or story overview' 
            },
            cast: { 
              type: 'array',
              items: { type: 'string' },
              description: 'Main cast members mentioned'
            },
            genre: { 
              type: 'string',
              description: 'Genre or vibe (e.g., "Action", "Drama", "Thriller")' 
      ...
      function: {
        ...
        parameters: {
          ...
          properties: { 
            brand_name: { 
              type: 'string',
              description: 'Brand name to search communications for'
            },
            time_range_days: { 
              type: 'integer',
        ...
        parameters: {
          ...
          properties: { 
            brand_names: { 
              type: 'array',
              items: { type: 'string' },
              description: 'Specific brand names to create pitches for'
            },
            project_name: { 
              type: 'string',
              description: 'Production/project name if mentioned' 
            },
            synopsis: { 
              type: 'string',
              description: 'Plot description if provided'
            }
          },
...
- "Show me emails about Nike" → get_brand_activity`;
    ...
    if (toolCall) {
      ...
      // GPT now extracts all parameters directly - no post-processing needed!
      // The structured outputs handle project names, cast, synopsis, etc. automatically
...
async function runQuickMatchOrchestrator(
  ...
  // STEP 1: Get project name - GPT already extracted it!
  let projectName = intent.args.project_name || knownProjectName;
  const akaName = intent.args.aka_name;
  const extractedSynopsis = intent.args.synopsis;
  const extractedCast = intent.args.cast;
  const extractedGenre = intent.args.genre;
  
  // Combine project name with aka if provided
  const fullProjectName = akaName ? `${projectName} aka ${akaName}` : projectName;
  
  if (!projectName) {
    add({ type: 'warning', text: '⚠️ No project name detected' });
...
export async function runBrandSearchOrchestrator(
  ...
  // Step 0: Get parameters extracted by GPT
  const projectName = intent.args.project_name || knownProjectName;
  const extractedSynopsis = intent.args.synopsis;
  const extractedCast = intent.args.cast;
  const extractedGenre = intent.args.genre;
  const requestedCategory = intent.args.requested_category;
  
  // Extract user-suggested brands if any
...
// ... inside generateCreativeContent function
// ...
} else {
...
Cast: ${Array.isArray(partnershipData?.cast) ? partnershipData.cast.join(', ') : partnershipData?.cast || 'Not specified'}
</PROJECT_DETAILS>`; // <-- FIX #1: ADD BACKTICK HERE
...
</OUTPUT_FORMAT>`; // <-- FIX #2: ADD BACKTICK HERE
