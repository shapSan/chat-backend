...
\`\`\``;
  ...
  // Sort by relevance score and limit to top brands
  const allRankedBrands = Array.from(brandMap.values())
    .sort((a, b) => (b.relevanceScore || 0) - (a.relevanceScore || 0))
    .slice(0, 15);
  
  // Select top 2 brands for creative pitches
  const brandsForPitching = allRankedBrands.slice(0, 2);
...
async function searchCommunicationsForClues(partnershipData, { add }) {
  // Generate AI business categories for broad searches
  let businessCategories = [];
  if (openAIApiKey && partnershipData.synopsis) {
    try {
      const categoryPrompt = `Based on the project '${partnershipData.title || 'production'}' with synopsis: "${(partnershipData.synopsis || '').slice(0, 200)}", provide 3-4 business categories or brand types that would be relevant for product placement. Focus on industries and product categories. Respond with ONLY a comma-separated list.`;
      ...
      if (categoryResponse.ok) {
        ...
        let categoriesText = categoryData.choices?.[0]?.message?.content?.trim().replace(/["'*\.]/g, '');
        if (categoriesText) {
          businessCategories = categoriesText.split(',').map(c => c.trim()).filter(Boolean);
        }
      }
    } catch (error) {
      console.error('Failed to generate business categories:', error.message);
    }
  }
  
  const searchTerms = uniqShort([partnershipData.title, ...businessCategories, ...(partnershipData.cast || []).slice(0, 2)].filter(Boolean), 7);
  add({ type: 'info', text: `🔍 Searching meetings & emails for: ${searchTerms.join(', ')}` });

  const [ffResult, o365Result] = await Promise.allSettled([
    firefliesApiKey ? searchFireflies(searchTerms, { limit: 15 }) : Promise.resolve({ transcripts: [] }),
    o365API.searchEmails ? o365API.searchEmails(searchTerms, { limit: 15 }) : Promise.resolve({ emails: [] })
  ]);
  ...
  add({ type: 'result', text: `📊 Found ${meetings.length} relevant meetings & ${emails.length} emails` });
...
async function fetchBrandsByName(brandNames, { add }) {
  ...
  // Search for each brand name - limit to 15 to avoid overload
  const searchPromises = brandNames.slice(0, 15).map(brandName => 
    hubspotAPI.searchBrands({
      query: brandName,
      limit: 5, // Increased from 3 to get better matches
      properties: ['brand_name', 'main_category', 'product_sub_category__multi_', 'client_status', 
                   'client_type', 'partnership_count', 'deals_count', 'hs_lastmodifieddate',
                   'secondary_owner', 'specialty_lead', 'one_sheet_link']
    }).catch((error) => {
      console.error(`Failed to search for brand: ${brandName}`, error.message);
      return { results: [] };
    })
  );
  ...
  searchResults.forEach((result, index) => {
    const searchedName = brandNames[index].toLowerCase();
    result.results?.forEach(brand => {
      if (brand.id && brand.properties?.brand_name) {
        ...
        if (isMatch && !brandMap.has(brand.id)) {
          ...
          const transformedBrand = {
            ...
            source: 'hubspot',
            tags: ['💬 Found in Comms'],
            reason: `Actively discussed in recent communications about ${brandNames[index]}`,
...
async function fetchCuratedBrandBuckets(partnershipData, { add }) {
  ...
  const communications = await (async () => {
    // Generate business categories using AI for smarter search
    let businessCategories = [];
    if (openAIApiKey && partnershipData.synopsis) {
      try {
        const categoryPrompt = `Based on the project '${partnershipData.title || 'production'}', which is a ${(partnershipData.synopsis || '').slice(0, 100)}..., provide the top 2 most likely business-related search terms for finding relevant meetings. Respond with ONLY a comma-separated list.`;
        const categoryResponse = await fetch('https://api.openai.com/v1/chat/completions', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${openAIApiKey}` },
          body: JSON.stringify({
    ...
    if (brandNamesForSearch.length === 0 && businessCategories.length === 0) return { meetings: [], emails: [] };

    // Combine brand names with business categories for comprehensive search
    const commsTerms = uniqShort([...brandNamesForSearch, ...businessCategories], 7);
...
\`\`\``;
  ...
  allRankedBrands = allRankedBrands.map(brand => {
    // Re-calculate relevance score using the same logic as tagAndCombineBrands
    let score = 0;
    const isContextMatch = brand.tags?.includes('🎯 Category Match') || 
                           brand.tags?.includes('🎯 Genre Match') || 
                           brand.tags?.includes('🎯 Perfect Context Match');
    const isActiveClient = brand.tags?.includes('🔥 Active Client');
    const isReIgnition = brand.tags?.includes('📈 Win-Back') || brand.tags?.includes('📈 Re-Ignition Opportunity');
    const isWildcard = brand.tags?.includes('🚀 Wild Card');
    
    // Apply the discovery bonus scoring system
    if (isContextMatch) {
      score += 100; // High base score for being a perfect contextual fit
    }
    if (isReIgnition) {
    ...
    if (isActiveClient) {
      score += 25; // Smaller bonus for existing active clients
    }
    if (isWildcard) {
      score += 30; // Bonus for creative wildcards
    }
    
    // Discovery bonus: huge bonus if it's a perfect fit but NOT an active client
    if (isContextMatch && !isActiveClient) {
      score += 50;
    }
    
    // Add kicker based on partnership count (max 10 points)
    const partnershipCount = parseInt(brand.partnershipCount || brand.partnership_count || 0);
    score += Math.min(10, partnershipCount);
    
    // Add bonus for recent activity
    if (brand.hasRecentActivity || brand.insight) {
      score += 15;
    }
  ...
  allRankedBrands = allRankedBrands.slice(0, Math.min(targetSize, allRankedBrands.length));
