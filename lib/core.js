  ...
  try {
    ...
    for (const selection of selections) {
      ...
      if (brand) {
        selectedBrands.push({
          ...brand,
          name: brand.properties?.brand_name || selection.name,
    ...
    while (finalBrands.length < 30 && brandsToAnalyze.length > finalBrands.length) {
      ...
      if (nextBrand) {
        finalBrands.push({
          ...nextBrand,
          name: nextBrand.properties?.brand_name || 'Unknown',
          category: nextBrand.properties?.main_category || 'General',
...
export async function runBrandSearchOrchestrator(
  ...
  // Step 1: Extract project name more intelligently
  let projectName = knownProjectName || (intent && intent.args && (intent.args.project_name || intent.args.production)) || '';
  
  // Try to extract project name from the beginning of the message if it looks like a title
  if (!projectName && userMessage) {
    // Look for common patterns:
    // 1. "Title Name" followed by description
    // 2. Title at the beginning before "A reboot" or "A sequel" etc.
    const titlePatterns = [
      /^([A-Z][\w\s&:,'-]+?)\s+(?:A|An|The)\s+(?:reboot|sequel|prequel|remake|adaptation|series|movie|film)/i,
      /^([A-Z][\w\s&:,'-]+?)\s*\n/,  // Title on its own line
      /^([A-Z][\w\s&:,'-]+?)\s+(?:is|follows|tells|about|starring)/i,
      /^"([^"]+)"/,  // Title in quotes
    ...
    for (const pattern of titlePatterns) {
      ...
      if (match && match[1]) {
        ...
        console.log('[runBrandSearchOrchestrator] Extracted project name from message start:', projectName);
        break;
      }
    }
    
    // Fallback: look for "for" pattern but more carefully
    if (!projectName) {
      const forMatch = /(?:for|on|about)\s+["']?([^"']{3,60})["']?/i.exec(userMessage);
      if (forMatch && !forMatch[1].toLowerCase().includes('production') && !forMatch[1].toLowerCase().includes('release')) {
        projectName = forMatch[1];
      }
    }
  }

  const extractedData = { title: projectName };
  let commsResults = { meetings: [], emails: [], brandHasEvidence: () => false };
  let extractedKeywords = [];
  
  // Extract more details from the user message
  if (userMessage) {
    // Extract cast (looking for "Anna Faris", "Regina King", "The Wayans", etc.)
    const castMatch = userMessage.match(/(?:starring|cast|with|return).*?([A-Z][a-z]+ [A-Z][a-z]+(?:,? (?:and |, )?[A-Z][a-z]+ [A-Z][a-z]+)*)/i);
    if (castMatch) {
      extractedData.cast = castMatch[1];
    }
    
    // Extract location
    const locationMatch = userMessage.match(/(?:in|at|filming in|shooting in)\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)?)/i);
    if (locationMatch && !locationMatch[1].toLowerCase().includes('development')) {
      extractedData.location = locationMatch[1];
    }
    
    // Extract release date
    const releaseDateMatch = userMessage.match(/release date of\s+([A-Za-z]+ \d+,? \d{4})/i);
    if (releaseDateMatch) {
      extractedData.releaseDate = releaseDateMatch[1];
    }
    
    // Extract genre if mentioned (for Scary Movie, it's horror-comedy)
    if (projectName && projectName.toLowerCase().includes('scary movie')) {
      extractedData.vibe = 'Horror-Comedy';
      extractedData.genre_production = 'Comedy';
    }
  }
  ...
  // If we didn't get a synopsis from HubSpot but have the user message with project details,
  // extract the synopsis from the user message
  let extractedSynopsis = extractedData.synopsis;
  if (!extractedSynopsis && userMessage && projectName) {
    // Try to extract everything after the project name as the synopsis
    const projectIndex = userMessage.toLowerCase().indexOf(projectName.toLowerCase());
    if (projectIndex !== -1) {
      const afterTitle = userMessage.substring(projectIndex + projectName.length).trim();
      // Clean up common starting words
      extractedSynopsis = afterTitle.replace(/^(A|An|The)\s+(reboot|sequel|prequel|remake|adaptation|series|movie|film)\s+of\s+the\s+franchise\.?\s*/i, '');
      // Take everything up to "Production" or "Awaiting" as the synopsis
      const endMarkers = ['Production Starting', 'Awaiting pages', 'Looking for', 'Budget:', 'Release Date:'];
      for (const marker of endMarkers) {
        const markerIndex = extractedSynopsis.indexOf(marker);
        if (markerIndex > 0) {
          extractedSynopsis = extractedSynopsis.substring(0, markerIndex).trim();
          break;
        }
      }
    }
  ...
  const finalPartnershipData = {
      ...
      synopsis: hasContent(partnershipData?.synopsis) ? partnershipData.synopsis : (extractedSynopsis || extractedData.synopsis),
  };
