...
async function runQuickMatchOrchestrator(
  ...
  if (selectedBrands && Array.isArray(selectedBrands) && selectedBrands.length > 0) {
    ...
    // Get partnership data for context
...
export async function runBrandSearchOrchestrator(
  ...
  if (selectedBrands && Array.isArray(selectedBrands) && selectedBrands.length > 0) {
    ...
    // Extract project info from known data or session
    let projectName = knownPartnershipData ? await extractProjectName(knownPartnershipData) : null;
    if (!projectName) projectName = knownProjectName;
  ...
  // CRITICAL FIX: If we have partnership data, use it immediately
  if (knownPartnershipData) {
    console.log('[runBrandSearchOrchestrator] Have knownPartnershipData, will use it directly');
    // Don't try to extract or detect project - we already have the data
  }
  ...
  // CRITICAL FIX: Trust the session/GPT over manual extraction
  let projectName = knownProjectName;  // Start with session context if available
  
  // Only override with GPT-extracted if it's a new project (not a follow-up)
  if (extractedProjectName && !knownProjectName) {
    projectName = extractedProjectName;
  }
  
  // If we have partnership data but no project name, extract it from the data
  if (!projectName && knownPartnershipData) {
    projectName = await extractProjectName(knownPartnershipData);
    if (projectName) {
      console.log('[runBrandSearchOrchestrator] Extracted project name from partnership data:', projectName);
    }
  }
...
export async function handleClaudeSearch(
  ...
  runId,
  onStep = () => {},
  knownPartnershipData,  // Partnership data from frontend
  selectedBrands  // NEW: User-selected brands from frontend
) {
  ...
  try {
    ...
    // Check if we have knownPartnershipData from the frontend first
    if (knownPartnershipData) {
      console.log('[handleClaudeSearch] Using knownPartnershipData from frontend');
      // Extract project name from the provided data
      const extractedName = await extractProjectName(knownPartnershipData);
      if (extractedName) {
        currentProjectName = extractedName;
        currentProjectContext = normalizePartnership(knownPartnershipData);
        ...
        console.log('[handleClaudeSearch] Set project from frontend data:', currentProjectName);
      }
    }
    
    // If no knownPartnershipData, load from session
    if (!currentProjectContext) {
      const sessionContext = await loadSessionContext(sessionId);
      currentProjectContext = sessionContext?.partnershipData || null;
      currentProjectName = sessionContext?.projectName || null;
    ...
    // ONLY check for new project if we don't have partnership data from frontend
    let newProjectInfo = null;
    if (!currentProjectContext) {
      newProjectInfo = await detectNewProject(userMessage, openAIApiKey);
      ...
      // Skip new project detection when we already have partnership data
      console.log('[handleClaudeSearch] Skipping new project detection - using provided partnership data');
      add({ type: 'info', text: `📌 Using existing project: "${currentProjectName}"` });
    }
    
    if (!currentProjectContext && !newProjectInfo && lastProductionContext) {
  ...
  switch (intent.tool) {
    case 'quick_match_brands': {
      ...
      return await runQuickMatchOrchestrator(intent, {
        ...
        selectedBrands  // Pass selected brands
      });
    }
