  ...
  try {
    switch (intent.tool) {
      case 'find_brands': {
        ...
        // 2. Top 2 brands only will be passed to the AI for detailed write-ups
        
        const fullBrandList = finalBrands;  // Keep all ~15-20 brands for the picker
        const top2BrandsForAI = finalBrands.slice(0, 2);  // Only top 2 for detailed AI write-up
        
        add({ type: 'process', text: `📝 Preparing dual response: ${top2BrandsForAI.length} brands for detailed write-up, ${fullBrandList.length} brands for picker` });
        ...
        return {
          ...
          organizedData: {
            ...
            brandsForDetailedResponse: top2BrandsForAI,  // TOP 2 ONLY for AI to write detailed sections
            pools: {
