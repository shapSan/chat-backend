...
async function generateCreativeContent(topBrands, { add, actualProjectName, search_term, partnershipData, communications }) {
  ...
  const enrichedBrandsWithContent = topBrands.map((originalBrand) => {
    ...
    // UPDATED LOGIC: Handle both string and array formats for integration ideas
    let integrationIdeas = '';
    if (Array.isArray(insights.integrationIdeas) && insights.integrationIdeas.length > 0) {
      integrationIdeas = insights.integrationIdeas.join(' ');
    } else if (typeof insights.integrationIdeas === 'string') {
      integrationIdeas = insights.integrationIdeas;
    } else {
      integrationIdeas = 'Strategic brand placement opportunities identified.';
    }
    ...
    return {
      ...
      deepInsights: {
        ...
        integrationIdeas: integrationIdeas,
