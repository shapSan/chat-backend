...
/* =========================
 * MAIN EXPORT: handleClaudeSearch
 * ======================= */

/**
 * Main entry point for brand search orchestration
 * This function routes to the appropriate orchestrator based on intent
 */
export async function handleClaudeSearch(
  ...
  console.log('[handleClaudeSearch] Detected intent:', intent);

  // If find_brands intent, use the brand orchestrator
  if (intent.tool === 'find_brands') {
    // IMPORTANT FIX: Extract the actual project name from the message, not the synopsis
    let actualProjectName = knownProjectName;
    
    // If knownProjectName looks like a synopsis (too long or contains descriptive text), try to extract the real title
    if (knownProjectName && knownProjectName.length > 100) {
      console.log('[handleClaudeSearch] knownProjectName appears to be synopsis, extracting title...');
      
      // Look for patterns like "Title: X" or "Project: X" in the message
      const titleMatch = userMessage.match(/(?:title|project|movie|film|show|series)\s*[:\-]?\s*["']?([^"'\n]{1,50})["']?/i);
      if (titleMatch) {
        actualProjectName = titleMatch[1].trim();
        console.log('[handleClaudeSearch] Extracted title from message:', actualProjectName);
      } else if (lastProductionContext) {
        // Try to extract from last production context
        const contextMatch = lastProductionContext.match(/^([^.\n]{1,50})/); // First sentence/phrase
        if (contextMatch) {
          actualProjectName = contextMatch[1].trim();
          console.log('[handleClaudeSearch] Extracted title from context:', actualProjectName);
        }
      } else {
        // Fallback: use "Untitled Project"
        actualProjectName = "Untitled Project";
        console.log('[handleClaudeSearch] Using fallback title:', actualProjectName);
      }
    }
    
    return await runBrandSearchOrchestrator(intent, {
      add,
      knownProjectName: actualProjectName, // Pass the cleaned project name
      userMessage,
      mcpThinking,
      conversationContext
    });
  ...
  if (intent.tool === 'get_brand_activity') {
    // Implement brand activity search
    add({ type: 'info', text: 'Searching for brand activity...' });
    // ... implementation ...
    return {
      mcpThinking,
      organizedData: {
...
\`\`\``;
  ...
  const bucketQuotas = { 'Category Match': 6, 'Win-Back': 5, 'Active Client': 5, 'Discovery': 5, 'Wild Card': 6 };
  ...
  const winBackBrands = allRankedBrands.filter(b => b.tags?.some(tag => tag.includes('📈 Win-Back')));
  const categoryMatchBrands = allRankedBrands.filter(b => b.tags?.some(tag => tag.includes('🎯 Category Match')));
  const wildCardBrands = allRankedBrands.filter(b => b.tags?.includes('🚀 Wild Card'));
  const activeClientBrands = allRankedBrands.filter(b => b.tags?.includes('🔥 Active Client'));
  
  if (winBackBrands.length > 0) { brandsForPitching.push(winBackBrands[0]); usedIds.add(winBackBrands[0].id); }
  else if (categoryMatchBrands.length > 0) { const brand1 = categoryMatchBrands.find(b => !usedIds.has(b.id)); if (brand1) { brandsForPitching.push(brand1); usedIds.add(brand1.id); } }
  
  if (wildCardBrands.length > 0) { const brand2 = wildCardBrands.find(b => !usedIds.has(b.id)); if (brand2) { brandsForPitching.push(brand2); usedIds.add(brand2.id); } }
  else if (activeClientBrands.length > 0) { const brand2 = activeClientBrands.find(b => !usedIds.has(b.id)); if (brand2) { brandsForPitching.push(brand2); usedIds.add(brand2.id); } }
  
  if (brandsForPitching.length < 2) { for (const brand of allRankedBrands) { if (!usedIds.has(brand.id)) { brandsForPitching.push(brand); if (brandsForPitching.length >= 2) break; } } }
...
async function searchCommunicationsForClues(partnershipData, { add }) {
  ...
  // Execute communications searches in parallel with proper timeframe
  if (primarySearchTerms.length > 0) {
    add({ type: 'info', text: `🔍 Executing parallel search for: ${primarySearchTerms.join(', ')}` });
    const DAYS = 90; // Rolling window for communications search
    const fromDate = new Date(Date.now() - DAYS * 24 * 60 * 60 * 1000).toISOString();
    
    const [ffResult, o365Result] = await Promise.allSettled([
      firefliesApiKey ? searchFireflies(primarySearchTerms, { limit: 10, fromDate }) : Promise.resolve({ transcripts: [] }),
