...
// Normalizes "integration" / "integrationIdeas" into a single display string.
function toIntegrationIdeasString(value) {
  const raw = (value?.integrationIdeas ?? value?.integration ?? value);
...
Return this exact JSON structure:
{
  ...
  const enhancedBrands = brandsForPitching.map((brand, idx) => {
    ...
    const integrationIdeasStr = toIntegrationIdeasString(insights);
    return {
      ...brand,
      deepInsights: {
        ...insights,
        // guarantee SINGLE string for the UI
        integrationIdeas: integrationIdeasStr,
      },
      // Use the AI's strategic fit as the primary reason
      reason: insights.strategicFit || brand.reason,
      // keep a flat copy if anything else reads it
      integrationIdeas: integrationIdeasStr,
...
async function generateCreativeContent(topBrands, { add, actualProjectName, search_term, partnershipData, communications }) {
  ...
  const enrichedBrandsWithContent = topBrands.map((originalBrand) => {
    ...
    // Robust normalization (also handles legacy "integration")
    const integrationIdeas = toIntegrationIdeasString(insights) 
      || 'Strategic brand placement opportunities identified.';
    
    const whyItWorks = insights.strategicFit || insights.whyItWorks || 'Aligns with production themes.';
    ...
    return {
      ...
      deepInsights: {
        // Pass the correct format to the BrandCard
        integrationIdeas,
        whyItWorks,
        insight: hbInsights,
        strategicFit: whyItWorks
      },
      name: brandName,
  ...
  // Generate a more detailed summary for the frontend
  const brandNames = enrichedBrandsWithContent.map(b => b.brand_name || b.name).filter(n => n && n !== 'Unknown Brand');
  const finalReplyText = `Generated brand integration ideas for ${brandNames.join(' and ')} for ${actualProjectName || 'your project'}.`;
