...
async function runQuickMatchOrchestrator(
  ...
  if (selectedBrands && Array.isArray(selectedBrands) && selectedBrands.length > 0) {
    ...
    const creativeResult = await generateCreativeContent(selectedBrands.slice(0, 2), {
      add,
    ...
    return prepareFrontendPayload({
      ...
      allRankedBrands: selectedBrands,
      mcpThinking,
...
export async function runBrandSearchOrchestrator(
  ...
  if (selectedBrands && Array.isArray(selectedBrands) && selectedBrands.length > 0) {
    ...
    const enhancedBrands = selectedBrands.map((brand, idx) => {
