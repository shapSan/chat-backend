...
export async function saveSessionContext(sessionId, projectContext) {
  ...
  try {
    ...
    // CRITICAL FIX: Create a clean copy of partnershipData to avoid data contamination
    // Only keep the essential fields we actually need
    ...
    if (projectContext.partnershipData) {
      ...
      cleanPartnershipData = Object.create(null);
      
      // Only copy specific fields we need - no spreading!
      cleanPartnershipData.id = data.id;
      cleanPartnershipData.partnership_name = data.partnership_name || data.name;
      cleanPartnershipData.title = data.title || data.partnership_name || data.name;
      
      // Production details
      cleanPartnershipData.synopsis = data.synopsis;
      cleanPartnershipData.genre_production = data.genre_production || data.genre || data.vibe;
      cleanPartnershipData.vibe = data.vibe || data.genre_production || data.genre;
      cleanPartnershipData.production_type = data.production_type;
      cleanPartnershipData.rating = data.rating || data.movie_rating || data.tv_ratings;
      
      // Cast - CRITICAL: Only take the main_cast field directly from current data
      // Do NOT use fallbacks or alternatives that might contain old data
      if (data.main_cast && typeof data.main_cast === 'string') {
        cleanPartnershipData.main_cast = data.main_cast;
        cleanPartnershipData.cast = data.main_cast; // Mirror for compatibility
      }
      
      // Dates
      cleanPartnershipData.start_date = data.start_date || data.startDate || data.production_start_date;
      cleanPartnershipData.startDate = data.startDate || data.start_date || data.production_start_date;
      cleanPartnershipData.production_start_date = data.production_start_date || data.start_date || data.startDate;
      cleanPartnershipData.release__est__date = data.release__est__date || data.releaseDate || data.release_est_date;
      cleanPartnershipData.releaseDate = data.releaseDate || data.release__est__date || data.release_est_date;
      
      // Location
      cleanPartnershipData.location = data.location;
      cleanPartnershipData.plot_location = data.plot_location;
      cleanPartnershipData.partnership_setting = data.partnership_setting;
      cleanPartnershipData.storyline_location__city_ = data.storyline_location__city_;
      
      // Other metadata
      cleanPartnershipData.distributor = data.distributor;
      cleanPartnershipData.budget = data.budget;
      cleanPartnershipData.hollywood_branded_fee = data.hollywood_branded_fee;
      
      // Remove any null/undefined fields to keep it clean
      Object.keys(cleanPartnershipData).forEach(key => {
        if (cleanPartnershipData[key] === null || cleanPartnershipData[key] === undefined) {
          delete cleanPartnershipData[key];
        }
      });
    }
