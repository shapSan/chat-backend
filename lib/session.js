...
/**
 * Detect if the user's message signals a new project with detailed information
 * Returns null if this is a follow-up or simple query, or extracted project info if it's truly new
 * 
 * CRITICAL: This should ONLY return a new project if the message contains:
 * - Synopsis/plot description
 * - Detailed production information (studio, dates, cast, etc.)
 * - Multiple project attributes being defined
 * 
 * Simple queries like "Quick Match for X" or "Find brands for X" should NOT trigger this.
 */
export async function detectNewProject(userMessage, openAIApiKey) {
  // Strong signals that this is a NEW project with DETAILS (not just a name)
  const newProjectWithDetailsSignals = [
    /synopsis:\s*[""']?(.{20,})[""']?/i,  // Requires at least 20 chars of synopsis
    /starring:\s*(.+?)(?:and|,|\.|\n)/i,  // Cast information
    /plot:\s*[""']?(.{20,})[""']?/i,  // Plot description
    /about:\s*[""']?(.{30,})[""']?/i,  // Longer "about" text suggests synopsis
    /(?:production|filming)\s+starts?:\s*\d{4}/i,  // Production dates
    /(?:release|premiere)\s+date:\s*\d{4}/i,  // Release dates
  ];
  
  // Check for messages that contain actual project DETAILS (not just name + command)
  let hasDetailedInfo = false;
  for (const pattern of newProjectWithDetailsSignals) {
    if (pattern.test(userMessage)) {
      hasDetailedInfo = true;
      console.log('[session] Detected NEW project with detailed information');
      break;
    }
  }
  
  // Only proceed with AI extraction if we found detailed info
  if (hasDetailedInfo && openAIApiKey) {
        try {
...
DO NOT extract cast information - we only use verified cast data from our database.`;
          ...
          if (response.ok) {
            ...
            // STRICT CHECK: Only return if AI confirms it's new AND has substantial details
            if (extracted.isNewProject && extracted.title && (extracted.synopsis || extracted.genre)) {
              console.log('[session] AI confirmed new project with details:', extracted.title);
              return extracted;
            } else {
              console.log('[session] AI did not confirm new project (no substantial details)');
              return null;
            }
          }
        } catch (e) {
  ...
  // No detailed information found - treat as simple query, not new project
  console.log('[session] No detailed project information found - treating as query for existing/known project');
  return null;
