...
export async function deepDiveBrand(brandId, partnershipData, options = {}) {
  ...
  if (typeof brandId === 'object' && brandId !== null) {
    ...
    brand = brandId;
    console.log('[deepDiveBrand] Using provided brand object:', brand.brand_name || brand.name);
  } else {
    // Need to fetch from HubSpot
    // Check if brandId looks like a name (contains spaces or letters) vs ID (only numbers)
    const isNumericId = /^\d+$/.test(String(brandId));
    
    try {
      if (isNumericId) {
        // Search by ID
        console.log('[deepDiveBrand] Fetching brand by ID:', brandId);
        ...
        // Search by name
        console.log('[deepDiveBrand] Fetching brand by name:', brandId);
        const searchResult = await hubspotAPI.searchBrands({
          query: String(brandId),
          limit: 1,
          properties: BRAND_FIELDS
        });
