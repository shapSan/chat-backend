...
async function generateDeepInsights(hubspotData, marketContext, commsData, isLite = false) {
  ...
  const prompt = isLite ? 
    ...
    Generate creative integration ideas for this pairing.
    
    CRITICAL: Return ONLY a valid JSON object with these exact keys:
    {
      "strategicFit": "[Your analysis of why this brand fits the film - max 20 words]",
      "integrationIdeas": [
        "[First specific integration idea - max 20 words]",
        "[Second specific integration idea - max 20 words]"
      ],
      "insight": "[Data-driven insight about the partnership - max 15 words]"
    }
    
    DO NOT return the template above. Generate ACTUAL integration ideas and insights.` :
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (isLite) {
        ...
        console.log('[generateDeepInsights] Raw AI response for LITE:', content);
        const parsed = extractJson(content);
        console.log('[generateDeepInsights] Parsed JSON:', JSON.stringify(parsed, null, 2));
        return {
