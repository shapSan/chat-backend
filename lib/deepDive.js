...
async function searchCommunications(brand, partnershipData) { // Use the full brand object
  ...
  if (productionName) searchTerms.add(productionName);
  
  const genre = partnershipData.genre_production || partnershipData.vibe;
  if (genre) {
    genre.split(';').forEach(g => searchTerms.add(g.trim()));
  }
  
  const finalSearchTerms = Array.from(searchTerms).filter(Boolean); // Convert back to array
  results.keywords = finalSearchTerms;
  
  // Search only if we have terms
  if (finalSearchTerms.length === 0) return results;
...
async function getMarketContext(brandName, isLite = false) {
  ...
  try {
    ...
    if (searchResults.results?.length > 0) {
      // ADD THIS LOG
      console.log(`[getMarketContext] ✅ Success: Found market context for "${brandName}"`);
      return {
