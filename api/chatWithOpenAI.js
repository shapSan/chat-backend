...
Return ONLY valid JSON, no other text.`;
    ...
    // Budget ranges
    const highBudgetBrands = topBrands.filter(b => {
      const budget = b.budget.toLowerCase();
      return budget.includes('
...
}) && (budget.includes('m') || parseInt(budget.replace(/\D/g, '')) > 100000);
    });
    if (highBudgetBrands.length > 0) {
      mcpThinking.push(`${highBudgetBrands.length} brands with significant budgets`);
    }
    
    // Categories represented
    const categories = [...new Set(topBrands.map(b => b.category).filter(c => c && c !== 'General'))];
    if (categories.length > 0) {
      mcpThinking.push(`Categories: ${categories.slice(0, 5).join(', ')}`);
    }
    
    // Add meeting mentions
    if (meetingData.records && meetingData.records.length > 0) {
      const meetingMentions = [];
      meetingData.records.forEach(meeting => {
        const title = meeting.fields['Title'] || 'Untitled Meeting';
        const summary = meeting.fields['Summary'] || '';
        
        // Check if any of the top brands are mentioned in the meeting
        topBrands.forEach(brand => {
          if (summary.toLowerCase().includes(brand.name.toLowerCase()) || 
              title.toLowerCase().includes(brand.name.toLowerCase())) {
            meetingMentions.push(`${brand.name} in "${title}"`);
          }
        });
      });
      
      if (meetingMentions.length > 0) {
        mcpThinking.push(`Mentions: ${meetingMentions.slice(0, 3).join(', ')}`);
      } else if (meetingData.records.length > 0) {
        // Show general meeting context even if no specific brand mentions
        mcpThinking.push(`${meetingData.records.length} related meetings found`);
      }
    }
