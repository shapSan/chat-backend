    ...
    if (isPartnershipQuery) {
      ...
      if (partnershipsData.results?.length > 0) {
        ...
        const partnerships = partnershipsData.results.map(p => ({
          ...
          projectUrl: `https://www.google.com/search?q=${encodeURIComponent(p.properties.partnership_name || p.properties.production_name || p.properties.name || '')}`,
      ...
      return {
        organizedData: {
          ...
          production: {
            ...
            projectUrl: `https://www.google.com/search?q=${encodeURIComponent(title)}`
          },
